...
The published baseline checked on 2026-09-21 is **`create-mono-agent@0.22.0`**. Projects, tags, persistent subagents, browser-first setup, and the loopback/explicit-sharing console defaults are included in that release. This README also follows current source, where the framework, Phoenix/OTLP, Supermemory, and terminal-renderer retirements remain unreleased. Use the [versioned release notes](https://github.com/robertsreberski/mono-agent/releases/tag/v0.22.0) when evaluating an installed version, and read [Release status](./docs/reference/release-status.md) for the verified source-only boundary.
...
- **Memory (optional)** — local Lite, Journal, and BuJo tiers span lexical recall, semantic indexing, and strict model-guided curation. Programmatic hosts can inject a neutral custom `MemoryStore`. See [Memory](./docs/memory/index.md) and [Tiers and custom stores](./docs/memory/backends-comparison.md).
...
- **The web console has no application login.** Both the published 0.22.0 release and current source bind a fresh console to `127.0.0.1:5050`; macOS managed start publishes a mono-agent-owned Tailscale route only with `--share-tailnet` and may re-verify an existing exact owned route. Widening is explicit (`--host <addr>`), the foreground `mono-agent web run --loopback` console touches no proxy configuration, and neither mode inspects or removes other proxies, tunnels, or routes. Existing managed definitions can also preserve their configured host. Anyone who can reach the console operates the discovered agents and can complete provider sign-in, so read [Release status](./docs/reference/release-status.md) and [the console's security boundary](./docs/observability/web-console.md#security-boundary-trusted-network-no-login) before widening the bind or publishing a route.
