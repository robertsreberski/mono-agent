...
The old `mono-agent recipes …` / `--recipe <id>` surface still works as a deprecated alias until its `v2.0.0` removal — retired recipes map to the preset that replaced them. See [`docs/reference/recipes.md`](./docs/reference/recipes.md) for the presets, capability modules, tools/no-tools guardrail, and migration map, and [`docs/reference/deprecations.md`](./docs/reference/deprecations.md) for the canonical deadline and permanent-compatibility decisions.
