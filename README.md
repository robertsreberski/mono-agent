...
- **The web console has no application login.** A fresh install binds `127.0.0.1:5050`, and the foreground console (`mono-agent web run --loopback`) touches no proxy configuration. Widening is explicit (`--host <addr>`), and managed macOS startup re-verifies an existing mono-agent-owned Tailscale Serve route or publishes a new one only with `--share-tailnet`; other proxies, tunnels, and routes are not inspected. Because anyone who can reach the console operates the discovered agents and can complete provider sign-in, read [the console's security boundary](./docs/observability/web-console.md#security-boundary-trusted-network-no-login) before widening the bind or publishing a route.
