...
Local JSONL artifacts are the completed-run fallback only after the terminal write succeeds; before then the on-disk record is only the start snapshot. At `start()`, the recorder independently replaces an empty events file and a `running` summary, then buffers later events in RAM. Terminal `finish()`/`fail()` independently replaces the complete events file first and the summary second. A crash before that terminal persistence can lose buffered events, and stale reconciliation can report only the data already on disk; the artifacts are not an in-flight or crash-safe source of truth.
