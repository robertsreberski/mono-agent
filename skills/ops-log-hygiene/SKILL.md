...
- The separately-managed `~/ops-agents` fleet writes to
  `~/Library/Logs/ops-agents/<service>.log` (9 services) — a different namespace.
...
grew to **1.23 GB** (`~/.mono-agent/logs/com.mono-agent.example-instance-00000001.err.log.1`
...
`OPS_BRAIN_ATTENTION_TOKEN must contain at least 32 characters.` lines
(`~/Library/Logs/ops-agents/brain-core.log:45-152`, ~18 min minimum) — before a
