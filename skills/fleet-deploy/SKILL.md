---
name: fleet-deploy
description: Deploy repo changes to the live launchd mono-agent fleet (~/example-instance, ~/ops-agents/orchestrator) and restart/verify them. Use when asked to "deploy", "restart the agents", "get this live", or after merging changes the fleet should run.
---
...
`activity-summary`, `deep-research`, `finances`, `inner-child`, `orchestrator`,
`feed-watcher`, `personal-agent`, `message-sweep`, and `transcription`. A missing PID,
...
|---|---|---|
| `~/example-instance` | `mono-agent.config.json` / `com.mono-agent.example-instance-00000001` | Telegram (allowlist 183676192), webhook, OpenAI API :4312/v1, 4 cron jobs, bujo memory, piAuthPath `~/.pi/personal-agent/auth.json` |
| `~/ops-agents/orchestrator` | `mono-agent.config.json` / `com.mono-agent.orchestrator-2146e3d3` | Slack Socket Mode, OpenAI API :4311/v1, journal memory, piAuthPath `~/.pi/ops-agent/auth.json`, working dir `.ops-agent/` (NOT `.mono-agent/`) |
...
```bash
cd ~/ops-agents/orchestrator && mono-agent restart   # or: npm run restart
cd ~/ops-agents/orchestrator && mono-agent validate 2>&1 | tail -45
cd ~/example-instance && mono-agent restart 2>&1 | tail -25
cd ~/example-instance && mono-agent validate 2>&1 | tail -45
mono-agent status
...
```bash
FLEET_LABELS='com.mono-agent.activity-summary-0680cd0b,com.mono-agent.deep-research-cd0b9a0d,com.mono-agent.finances-e9c073d7,com.mono-agent.inner-child-fdfc3392,com.mono-agent.orchestrator-2146e3d3,com.mono-agent.feed-watcher-e537b146,com.mono-agent.example-instance-00000001,com.mono-agent.message-sweep-aa87c1b8,com.mono-agent.transcription-f4a742c8'
...
- Orchestrator fallback `pi:opencode-go:glm-5.2` has no creds in
  `~/.pi/ops-agent/auth.json` — validate warns; pre-existing, not your bug.
