...
`personal-agent`.
The separately managed `~/ops-agents` services use their own launchd namespace
and status command; do not silently mix stopped OPS services into this gate. A missing PID,
...
| `~/example-instance` | `mono-agent.config.json` / `com.mono-agent.example-instance-00000001` | Telegram, webhook, OpenAI API :4312/v1, cron, bujo memory, restart last |
...
FLEET_LABELS='com.mono-agent.ambra-sleep-083399be,com.mono-agent.finances-e9c073d7,com.mono-agent.inner-child-fdfc3392,com.mono-agent.example-instance-00000001,com.mono-agent.transcription-f4a742c8'
