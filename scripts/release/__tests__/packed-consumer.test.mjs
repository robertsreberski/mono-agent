...
describe("packed consumer verification", () => {
  ...
  test("rejects an incompatible host Pi copy that rewires Pi Agent Core", () => {
    ...
    expect(() =>
      ...
      "Packed consumer resolved @earendil-works/pi-ai@0.80.8 from @earendil-works/pi-agent-core@0.87.1; expected 0.87.1",
    );
  });

  test("rejects a packed manifest that can float to a newer Pi runtime", () => {
    const fixture = packedDependencyFixture({ appPiRange: "^0.87.1" });

    expect(() =>
      assertPackedDependencyResolution(fixture.consumerDir, fixture.packages),
    ).toThrow(
      /Packed @mono-agent\/agent-app dependencies\.@earendil-works\/pi-ai must remain 0\.87\.1; found \^0\.87\.1/u,
    );
  });
  ...
  test("rejects an incompatible Pi AI nested under the pinned core", () => {
    ...
    expect(() =>
      ...
      /resolved @earendil-works\/pi-ai@0\.80\.8 from @earendil-works\/pi-agent-core@0\.87\.1; expected 0\.87\.1/u,
    );
  });
...
function packedDependencyFixture({
  appPiRange = "0.87.1",
  nestedCorePiVersion = "0.87.1",
  nestedRuntimePiVersion,
  rootPiVersion = "0.87.1",
} = {}) {
  ...
  const coreDir = writePackage(modulesDir, {
    ...
    version: "0.87.1",
    dependencies: { "@earendil-works/pi-ai": "^0.87.1" },
  });
  writePackage(modulesDir, {
