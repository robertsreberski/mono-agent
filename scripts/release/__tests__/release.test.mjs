...
describe("release graph validation", () => {
  ...
  test("rejects floating Pi dependencies in every publishable consumer", () => {
    ...
    try {
      ...
      expect(error.issues).toEqual([
        "@mono-agent/agent-app dependencies.@earendil-works/pi-ai must pin known-compatible version 0.87.0 exactly; found ^0.80.6",
        "@mono-agent/agent-runtime dependencies.@earendil-works/pi-agent-core must pin known-compatible version 0.87.0 exactly; found ~0.80.6",
        "@mono-agent/agent-runtime dependencies.@earendil-works/pi-ai must pin known-compatible version 0.87.0 exactly; found 0.80.8",
      ]);
    }
  });
