...
describe("rebuildFromMarkdown", () => {
  it("indexes every bullet across daily files, with no LLM, deterministically", async () => {
    ...
    writeFileSync(join(root, "daily", "2026-06-15.md"),
      '# 2026-06-15\n\n- – Example Operator prefers opt-in memory.  <!--mem id=01B type=note status=open salience=0.9 isInsight=1 created=2026-06-15T09:00:00.000Z refs=-->\n');
  ...
  it("ingests entities and relations from graph.jsonl after rebuild — no LLM called", async () => {
    ...
    writeFileSync(
      ...
      "# 2026-06-15\n\n- – Example Operator maintains mono-agent.  <!--mem id=EX1 type=note status=open salience=0.7 isInsight=0 created=2026-06-15T09:00:00.000Z refs=-->\n",
    );

    // Write graph.jsonl with an entity and a relation
    appendEntity(root, { id: "person:example-operator", name: "Example Operator", type: "person", createdAt: "2026-06-15T09:00:00.000Z" });
    appendEntity(root, { id: "project:mono-agent", name: "mono-agent", type: "project", createdAt: "2026-06-15T09:00:00.000Z" });
    appendRelation(root, { src: "person:example-operator", dst: "project:mono-agent", relation: "maintains", createdAt: "2026-06-15T09:00:00.000Z" });
    ...
    expect(db.getEntity("person:example-operator")).toMatchObject({ name: "Example Operator", type: "person" });
    expect(db.getEntity("project:mono-agent")).toMatchObject({ name: "mono-agent", type: "project" });

    // Relations were loaded
    expect(db.relationsFor("person:example-operator")).toContainEqual(
