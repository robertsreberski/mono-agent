...
describe("reconcile", () => {
  ...
  it("case 4 — refinement candidate + LLM says update → target text merged, count unchanged", async () => {
    const root = newRoot();
    const db = openDb(root);
    await seed(db, root, "UPD1", "example-operator prefers opt in memory capture");

    const merged = "example-operator prefers opt in memory capture with manual review";
    const llm = fakeLlm([["CLASSIFY", `{"action":"update","targetId":"UPD1","text":"${merged}"}`]]);
    const candidate: CandidateMemory = {
      type: "note",
      text: "example-operator prefers opt in memory capture and manual review",
      salience: 0.6,
      isInsight: false,
    };
    const before = db.count();
  ...
  it("case 5 — per-candidate isolation: a candidate whose write throws is skipped, others proceed", async () => {
    ...
    await db.upsert({
      id: "GHOST", type: "note", status: "open", text: "example-operator prefers opt in memory capture",
    ...
    const failing: CandidateMemory = { type: "note", text: "example-operator prefers opt in memory capture and review", salience: 0.6, isInsight: false };
