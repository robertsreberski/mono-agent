...
describe("writeIndex", () => {
  it("writes index.md with counts, top memories, and entities", async () => {
    ...
    await db.upsert({
      ...
      type: "note",
      status: "open",
      text: "Example Operator works best in the mornings",
      salience: 0.9,
      isInsight: false,
      createdAt: FIXED.toISOString(),
      accessCount: 0,
    ...
    db.upsertEntity({ id: "person:example-operator", name: "Example Operator", type: "person", createdAt: FIXED.toISOString() });
    ...
    expect(content).toMatch(/Example Operator works best|mono-agent is a personal/u);

    // Must contain Entities section with at least one entity.
    expect(content).toContain("## Entities");
    expect(content).toMatch(/Example Operator \(person\)|mono-agent \(project\)/u);
  });
