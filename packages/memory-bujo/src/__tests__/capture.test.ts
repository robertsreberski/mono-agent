...
describe("captureTurn", () => {
  it("distills → reconciles → extracts entities, mirrors to db AND graph.jsonl, links about edges", async () => {
    ...
    const llm = fakeLlm([
      [
        ...
        JSON.stringify({
          ...
          relations: [{ src: "person:example-operator", dst: "person:example-operator", relation: "self-reference" }],
        }),
      ],
      [
        "TEXT:",
        JSON.stringify([
          { type: "note", text: "Example Operator prefers opt-in memory capture", salience: 0.8, isInsight: false },
    ...
    const result = await captureTurn("Example Operator discussed the memory pipeline today", deps);
    ...
    const entity = db.getEntity("person:example-operator");
    expect(entity).toBeDefined();
    expect(entity?.name).toBe("Example Operator");
    expect(entity?.type).toBe("person");

    // Entity present in graph.jsonl
    const graph = readGraph(root);
    expect(graph.entities.some((e) => e.id === "person:example-operator")).toBe(true);

    // Relation present in graph.jsonl
    expect(graph.relations.some((r) => r.src === "person:example-operator" && r.relation === "self-reference")).toBe(true);

    // about edges: each added memory links to the extracted entity
    for (const action of result.actions) {
      if (action.kind === "add") {
        const edges = db.edges(action.id);
        expect(edges.some((e) => e.kind === "about" && e.dst === "person:example-operator")).toBe(true);
      }
    }
  });
  ...
  it("writes entities/relations to canonical graph.jsonl even when the db mirror fails (canonical-first)", async () => {
    ...
    const llm = fakeLlm([
      ...
      [
        "TEXT:",
        JSON.stringify([{ type: "note", text: "Example Operator maintains mono-agent", salience: 0.7, isInsight: false }]),
      ],
    ]);

    const deps: ReconcileDeps = { db: failingDb, root, llm, nextId: makeSeqNextId(), now: () => FIXED };
    await expect(captureTurn("Example Operator maintains mono-agent", deps)).resolves.toBeDefined();

    const graph = readGraph(root);
    expect(graph.entities.some((e) => e.id === "person:example-operator")).toBe(true);
    expect(graph.relations.some((r) => r.src === "person:example-operator" && r.relation === "maintains")).toBe(true);
  });
