...
export const CONFIG_ENV_KEYS = {
  ...
  "memory.operatorActions.enabled": "MONO_AGENT_MEMORY_OPERATOR_ACTIONS_ENABLED",
...
function buildMemorySection(input: BuildMonoAgentConfigViewInput): ConfigViewSection {
  ...
  const fields: ConfigViewField[] = [
    ...
    toField(env, {
      id: "memory.operatorActions.enabled",
      label: "Operator actions",
      value: memory.operatorActions?.enabled === true ? "on" : "off",
      jsonPresent: json.memory?.operatorActions?.enabled !== undefined,
    }),
  ];
