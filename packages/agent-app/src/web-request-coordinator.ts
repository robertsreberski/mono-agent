...
const defaults: Readonly<Record<string, readonly [number, number]>> = { searxng: [1, 2000], ollama: [1, 2000], duckduckgo: [1, 3000], startpage: [1, 3000], codex: [1, 0], fetch: [2, 500] } as const;
...
export function createHostWebRequestCoordinator(options: {
  ...
  return {
    ...
    async acquire(request) {
      ...
      if (!/^[a-z][a-z0-9-]{0,63}$/u.test(kind)) throw unavailable();
      const [max, spacing] = Object.hasOwn(defaults, kind) ? defaults[kind]! : [1, 2000];
...
function parseState(text: string): State {
  ...
  for (const [key, bucket] of Object.entries(value.buckets)) {
    if (!/^[a-z][a-z0-9-]{0,63}:[a-f0-9]{64}$/u.test(key)
