...
// Deliberately does NOT repeat "Read the mono-agent-configure skill": this
// prompt is prepended to every operator turn, on the user message no runtime
// drops, so the instruction re-fired a ReadSkill call per turn even when the
// body was still in the transcript. The opening prompt above asks for it once.
