...
function payload(value: Omit<PeerHandoff, "proof">): string {
  return JSON.stringify([value.version, value.caller, value.conversation, value.session, value.depth, value.digest]);
}

export async function makePeerHandoff(artifactDir: string, input: {
  caller: string; conversation: string; session: string; depth: number; text: string;
}): Promise<PeerHandoff> {
  ...
  const body = {
    ...
    session: input.session, depth: input.depth,
...
export async function verifyPeerHandoff(artifactDir: string, value: unknown, session: string, text?: string): Promise<PeerHandoff | undefined> {
  if (typeof value !== "object" || value === null || Array.isArray(value)) return undefined;
  const p = value as Partial<PeerHandoff>;
  if (Object.keys(p).sort().join(",") !== "caller,conversation,depth,digest,proof,session,version"
    || p.version !== 1 || p.session !== session || typeof p.caller !== "string"
