...
function fixture() {
  ...
  const mutate = vi.fn(async (operation: (value: SubagentExecutionOwnership) => void) => {
    const next = structuredClone(owner); operation(next); expect(isSubagentExecutionOwnership(next)).toBe(true); Object.assign(owner, next);
  });
  ...
  return { owner, request, commands, cleanup, release, cancel, mutate, setCompletion: (value: Promise<ProcessJobProcessResult>) => { completion = value; } };
}
