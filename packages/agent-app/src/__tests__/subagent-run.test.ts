...
describe("subagent confinement and context inheritance", () => {
  const SKILLS = [
    { name: "ops-context", description: "Read Example Corp work context." },
  ...
  it("inherits the parent's skill index and skills root, and renders the index into the child prompt", async () => {
    ...
    expect(prompt).toContain("- **ops-context** — Read Example Corp work context.");
...
describe("AskParent child policy and durable controller", () => {
  it.each(["allowed", "retry", "global-deny", "profile-deny", "stateless"])("applies %s policy", async (policy) => {
    ...
    try {
      ...
      const runtime = { run: async (_prompt: string, options: any) => {
        ...
        if (options.askParentController) {
          if (policy === "retry") {
            await expect(options.askParentController.submit({ question: "Scope?" })).rejects.toThrow("transient publication failure");
            expect((await handle.get("helper"))?.pendingQuestion).toBeUndefined();
          }
      ...
      expect(Boolean(calls[0].askParentController)).toBe(["allowed", "retry"].includes(policy));
      expect(calls[0].allowedTools.includes("AskParent")).toBe(["allowed", "retry"].includes(policy));
...
it.each([undefined, "short", "long"] as const)("forwards resolved %s retention to parent and child routes", async (cacheRetention) => {
  const providers = resolveJsonMonoAgentConfig({ cwd: "/repo", json: { runtime: { model: "anthropic:claude-sonnet-4-6" }, context: { identityPath: "IDENTITY.md" }, providers: { piNative: { ...(cacheRetention === undefined ? {} : { cacheRetention }), promptCacheDiagnostics: true } } } }).providers;
