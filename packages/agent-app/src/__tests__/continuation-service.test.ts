...
describe("durable continuation service", () => {
  ...
  it("issues exact reserved context, binds the reply destination, synthesizes once, and returns a delivery receipt", async () => {
    ...
    await putResult(claim, { status: "complete", findings: ["EX-1906"] });
    ...
    if (process.platform !== "win32") {
      ...
      expect((await stat(join(stateDir, "continuation-store-v2.json"))).mode & 0o077).toBe(0);
      expect((await stat(join(stateDir, "records-v2"))).mode & 0o077).toBe(0);
    }
    const recordFiles = await readdir(join(stateDir, "records-v2"));
    const persisted = (await Promise.all(recordFiles.map(async (file) =>
      await readFile(join(stateDir, "records-v2", file), "utf8")))).join("\n");
    ...
    await expect(service.health()).resolves.toMatchObject({
      storage: { format: "per-record-v2", records: 1, terminalTombstones: 1, compacted: 1 },
    });
  });
  ...
  it("migrates the legacy v1 ledger idempotently into owner-only per-record state", async () => {
    ...
    await expect(stat(join(stateDir, "continuations-v1.json"))).rejects.toMatchObject({ code: "ENOENT" });
    expect(await readdir(join(stateDir, "records-v2"))).toHaveLength(1);
    await expect(service.health()).resolves.toMatchObject({ storage: { format: "per-record-v2", records: 1 } });
  });

  it("fails closed when a partial legacy migration conflicts with an existing v2 record", async () => {
    const stateDir = fixtureDir("legacy-conflict");
    const first = await start({ stateDir });
    const claim = await issueAndClaim(first, "legacy-conflict-task");
    await first.stop();
    services.splice(services.indexOf(first), 1);
    const [recordFile] = await readdir(join(stateDir, "records-v2"));
    if (recordFile === undefined) throw new Error("expected migrated record");
    const record = JSON.parse(await readFile(join(stateDir, "records-v2", recordFile), "utf8")) as Record<string, unknown>;
  ...
  it("fails closed when the state directory cannot remain owner-only", async () => {
    ...
    await chmod(join(stateDir, "records-v2", recordFile), 0o644);
    await expect(startContinuationService({
      stateDir,
      autoProcess: false,
      synthesize: async () => ({ text: "x" }),
  ...
  it("holds exclusive state ownership across processes and recovers automatically after owner crash", async () => {
    ...
    await expect(recovered.health()).resolves.toMatchObject({ status: "healthy", storage: { format: "per-record-v2" } });
  });
});
...
async function issueAndClaim(service: ContinuationServiceHandle, taskKey: string): Promise<ClaimResponse> {
  ...
  return await claimContinuation(capability, taskKey, hash(taskKey));
}
