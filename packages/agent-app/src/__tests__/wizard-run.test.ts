...
describe("wizard production flow", () => {
  it("accepts canonical guided provider references and rejects malformed references", () => {
    ...
    expect(guidedModelRefProblem("ollama:qwen3:8b")).toBeUndefined();
    expect(guidedModelRefProblem("opencode:github-copilot:gpt-5.1")).toBeDefined();
    expect(guidedModelRefProblem("foo:bar")).toBeDefined();
    expect(guidedModelRefProblem("claude:")).toBeDefined();
    expect(guidedModelRefProblem("ollama:")).toBeDefined();
  });

  it("uses searchable model pickers, per-route efforts, and a clear creation review", async () => {
    ...
    expect(result.credentialStates).toEqual({
      "openai-codex": "credential_detected",
      anthropic: "auth_required",
    });
    ...
    expect(promptMock.autocompleteCalls[0]?.initialValue).toBe("openai-codex:gpt-5.6-sol");
  ...
  it("shows only advertised efforts and never offers none for Claude", async () => {
    ...
    promptMock.autocompleteAnswers.push("anthropic:claude-sonnet-5");
  ...
  it("treats a non-empty destination OPENCODE_API_KEY as durable credential detection", async () => {
    ...
    expect(result.status).toBe("answers");
    if (result.status !== "answers") return;
    expect(result.credentialStates).toEqual({ "opencode-go": "credential_detected" });
    ...
    expect(promptMock.selectCalls.some((call) => String(call.message).includes("store OPENCODE_API_KEY?")))
      .toBe(false);
    const review = promptMock.notes.find((note) => note.title === "Creation review")?.message ?? "";
    expect(review).toContain("opencode-go:kimi-k2.6: credential/sign-in detected");
  ...
  it("reviews an environment-selected Pi key as an owner-only .env write before creation", async () => {
    ...
    try {
      ...
      expect(result.status).toBe("answers");
      if (result.status !== "answers") return;
      expect(result.credentialStates).toEqual({ "opencode-go": "auth_required" });
  ...
  it("uses actual Escape keypresses to retry fallback effort and remove the latest fallback", async () => {
    ...
    promptMock.autocompleteAnswers.push(
      "openai-codex:gpt-5.6-terra",
      "anthropic:claude-sonnet-5",
      "anthropic:claude-sonnet-5", // effort Escape reopens fallback model #1
    ...
    expect(result.answers).toMatchObject({
      model: "openai-codex:gpt-5.6-terra",
      effort: "minimal",
      fallbacks: [{ model: "openai-codex:gpt-5.6-sol", effort: "xhigh" }],
    });
    expect(promptMock.selectCalls.filter((call) => call.message === "Reasoning effort for anthropic:claude-sonnet-5"))
      .toHaveLength(2);
  ...
  it("repairs capability details from Creation review without replaying unrelated prompts", async () => {
    ...
    const result = await runSetupRepairWizard({
      ...
      credentialStates: { "openai-codex": "auth_required" },
    ...
    expect(result.credentialStates).toEqual({ "openai-codex": "auth_required" });
    expect(result.moduleSecrets).toEqual({});
  ...
  it("returns Escape from seeded setup repair to recovery without changing state", async () => {
    ...
    await withTtyStdin(async () => {
      await expect(runSetupRepairWizard({
        cwd: "/tmp/agent",
        answers: defaultAnswers({ name: "Keep Me", model: "openai-codex:gpt-5.6-terra" }),
  ...
  it("returns a focused name edit directly to Creation review", async () => {
    ...
    const result = await runSetupRepairWizard({
      cwd: "/tmp/agent",
      initialStep: 0,
      answers: defaultAnswers({ name: "Original Partner", model: "openai-codex:gpt-5.6-terra" }),
      runProviderSetup: false,
      providerSetupSecrets: {},
      providerEnvironmentSecrets: {},
      piApiKeyPersistenceByProvider: {},
  ...
  it("preserves ephemeral provider state during a focused unchanged-model edit", async () => {
    ...
    const result = await runSetupRepairWizard({
      cwd: "/tmp/agent",
      initialStep: 1,
      answers: defaultAnswers({ model: "opencode-go:kimi-k2.6", effort: "medium" }),
      runProviderSetup: false,
