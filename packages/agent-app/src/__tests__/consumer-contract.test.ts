...
type ConsumerName = "personal-agent" | "ops-agent";
...
describe("golden consumer config contracts", () => {
  ...
  it("validates the ops-agent fixture without network access", async () => {
    const fetchSpy = disableNetwork();
    const fixture = await loadConsumerFixture("ops-agent");
