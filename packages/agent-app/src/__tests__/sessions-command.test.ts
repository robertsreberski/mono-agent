...
import { runSessions as runWeb } from "../sessions-command.js";
...
describe("runSessions", () => {
  ...
  it("normalizes an IPv6 wildcard advertisement to a usable bracketed loopback URL", async () => {
    ...
    expect(output).toContain("mono-agent sessions  →  http://[::1]:4599/");
    ...
    await runWeb(
      ...
      },
      {
        ...
        discoverNetworkAddresses: () => [
          ...
          { address: "100.64.0.42", kind: "tailscale" },
        ],
      },
    );

    expect(output).toContain("mono-agent sessions  →  http://127.0.0.1:4599/");
    expect(output).toContain("LAN       →  http://192.168.178.103:4599/");
    expect(output).toContain("Tailscale →  http://100.64.0.42:4599/");
  ...
  it("generates auth and prints concrete tokenized loopback, LAN, and Tailscale URLs", async () => {
    ...
    expect(output).toContain(`mono-agent sessions  →  http://127.0.0.1:4599/#token=${token}`);
    expect(output).toContain(`LAN       →  http://192.168.178.103:4599/#token=${token}`);
    expect(output).toContain(`Tailscale →  http://100.64.0.42:4599/#token=${token}`);
