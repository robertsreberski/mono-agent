...
await pruneVerificationScratch(verification, "observer-srt-", "observer-srt", { keep: keepScratch });
const root = await mkdtemp(resolve(verification, "observer-srt-"));
try {
  ...
  await removeVerificationScratch(root, { keep: keepScratch, label: "observer-srt" });
}
