...
- `@mono-agent/observability` for JSONL event artifacts, summaries, and trace-source registration; install the matching optional `@mono-agent/observability-phoenix` package for Phoenix OTLP export configured via `observability.exporters`.
