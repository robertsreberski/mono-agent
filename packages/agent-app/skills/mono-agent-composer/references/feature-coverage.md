...
| Configured local peer delegation | config | `peers.<name>.sourceId` maps a named local ACP target; explicitly allow `PeerAgent` in `tools.allowedTools`. `send` uses a caller-bound durable thread; `stop` cancels the active turn. Background requires a wake-capable origin and ProcessJobs; ACP-served calls are foreground only. Peer AskUser is unsupported (`interaction_required`), and peer text is not owner approval | `agent-app.peer-agent` |
...
| Persistent subagent instances | config | `subagents.instances` controls root, live capacity, idle TTL and total child turns. `Agent({persist:true})` creates an instance and `AgentManage` continues or closes it in the same conversation, or cooperatively stops detached work with exclusive `stop:true`; registries and Pi transcripts survive restarts. Disabling instances preserves stateless Agent. With ProcessJobs on an exact-conversation Pi route, persistent Agent/AgentManage accept background:true, return a durable receipt and wake once; unresolved timeout/cancellation reports childStillBusy:true and retains ownership until actual settlement | `runtime.subagent-instances` |
...
| Anthropic cache retention (default long, short opts out, model-supported, no guaranteed hit; 1h write 2× input, read 0.1×; short write 1.25×) | config | `providers.piNative.cacheRetention` (`MONO_AGENT_PI_CACHE_RETENTION`), short/long, default long overrides ambient Pi retention; verify measurement gates first | `runtime.prompt-cache-retention` |
...
| Optional BuJo capture focus and label-kind filter | config | `memory.capture.focus` (bounded operator guidance) and `memory.capture.only` (host-accepted `fact`, `preference`, `lesson` subset). Only automatic BuJo capture; unset keeps existing capture and explicit `Remember` writes are unaffected | `memory.capture-focus` |
...
| Local-first public-web research | config + auto | `tools.web.coordination: "host"` shares private admission/cooldown/quota state across agents under one OS user (default `process`); `mono-agent web-control status` inspects it. Broad primary queries and WebFetch `start_line`/`max_lines` avoid repeated work; Codex keeps a 10% quota reserve. `tools.web.search.maxRequestsPerRun` defaults to four actual provider requests per logical run, with cache/coalescing/cooldown/quota skips free. `tools.web.search.backend`, provider-scoped `searxng.endpoint` / `ollama.*`, and `codex.model` select strict providers or public no-credentials fallbacks; the default explicit chain is Parallel → local Ollama; keyless engines and native Node local search are opt-in, and `auto` is removed. Local search needs no endpoint or service; retired Hound endpoint fields/environment variables must be removed, and the `hound` provider value is a migration error. Cooldowns advance immediately and tell the model not to sleep or retry. `tools.web.fetch.provider` defaults to local, with optional Parallel extraction; local extraction performs no robots preflight; search/fetch `parallel.apiKeyEnv` names an optional credential variable. `tools.web.fetch.{render,browserCommand}` keeps deterministic static extraction as the default and optionally enables isolated agent-browser rendering; `render: "always"` is an explicit browser-first tool call under an `auto` config ceiling, and transient-fetch retry behavior is automatic rather than config-bearing | `runtime.web-research` |
...
| Operator stream endpoint (legacy `tui` protocol id) | config | `tui.{enabled,host,port,basePath,allowNonLoopback,apiKey}`; default on, loopback; `/v1/info` capability-gates exact AskUser and agent-owned cron routes without changing wire schema 1 | `tui.stream-endpoint` |
...
| Cron preflight gate (deterministic argv evaluated before the responder; `run:false` skips the firing, every failure fails open) | config + code | `cron.jobs[].preflight` (explicit argv; frontmatter takes one single-line JSON array) and `cron.jobs[].preflightTimeoutMs` / `MONO_AGENT_CRON_PREFLIGHT_TIMEOUT_MS` (default 5000, cap 60000), single-job `MONO_AGENT_CRON_PREFLIGHT_JSON`; programmatic `startCronAdapter({ preflight, onPreflight })` | `cron.preflight-gate` |
...
| Operator console (persistent live chat, attachments, thinking/tool insight, and authenticated agent routes) | cli | `mono-agent web`; agents serve the compatibility-id `tui` stream endpoint by default (`tui.enabled`, loopback); `mono-agent jobs` uses a separate owner capability | — |
| Always-on multi-agent web console (persistent conversations and same-thread quotes, one server-authoritative UUID-bearing Send path with durable read-only receipt recovery, agent-maintained semantic titles with permanent manual-rename precedence, durable process-job cards, live active-agent skill discovery with canonical `$skill-name` insertion, fixed compact/expanded rail, separate Chats and overview-backed Automations navigation, offline-agent filtering that always preserves pinned/selected agents, durable Web Push, exact structured AskUser reconciliation, stable read-only per-cron channels with bounded keyset history, attachments, per-conversation model/effort selection, SQLite-persisted per-agent defaults for new web conversations with one-click config revert, per-run requested/attempted/executed route and effective-effort attribution including nested subagents, remembered process-job revival selection, reasoning/tools, hidden telemetry-backed cumulative context usage, cancellation) | cli | `mono-agent web start\|stop\|restart\|status\|logs\|run`; Automations lists every agent-scoped overview job independently of the loaded Chats page and opens the stable `/agents/:sourceId/cron/:jobId` history route; saved snapshots stay visibly stale/read-only, next-run is live-authority-only, and truncated historical coverage is disclosed. Chats remains mixed and server-paginated until a server-side scoped follow-up. Confirmed cron mutations remain outside the list and require an operator key plus `cron.operatorActions.enabled`; process-job cards are source/thread-bound and update through the owner-authenticated operator proxy without a live browser; standalone process-job wake turns re-read the thread snapshot while an active Web Send targets the exact owned run or visibly queues on an older operator; fallback reasons and retries are bounded/sanitized before browser projection; skill autocomplete/browse reads the running agent's bounded registry and never sends on selection; bare `mono-agent web` is read-only status/help; a fresh install binds `127.0.0.1:5050`, `--host <addr>` widens explicitly, an installed service keeps its published bind, and `--share-tailnet` (macOS managed start/restart) publishes an owned Tailscale route while existing owned routes are re-verified; `--theme` selects the shell and `--name <label>` replaces the hostname-derived PWA/tab/rail label on start/restart/run, `--name -` restores the hostname default, and managed selections are persisted; package `@mono-agent/web`; no app authentication, so LAN/Tailnet reachability is the access boundary | — |
...
| Interactive setup wizard (preset/custom; exact `IDENTITY.md` → `## Role` prompt/outcome; walks name/Role → model/effort/fallbacks → optional-capabilities gate for channels/memory → capability details → tools → sandbox → creation review; declining the gate still runs the tool framing, the sandbox choice, and the default-No high-risk confirmation; Journal/BuJo explicitly choose Ollama or LM Studio service root/model/dimension/optional auth env using typed discovery and a real probe; macOS starts the background agent and prints the browser handoff `status` → `mono-agent web run --loopback` → `http://127.0.0.1:5050`) | cli | `mono-agent init` (no flags, on a TTY; `setup` alias); manual embedding entry still requires readiness probe; flags/non-TTY stay scaffold-only; unsupported platforms use manual configuration/foreground start plus the same browser console | — |
...
`AgentManage({id, message?, close?, stop?})`. Use `stop:true` alone for a queued/running
detached turn; only a proven `resumable:true` receipt allows ordinary continuation
or close. `stop_requested` preserves busy ownership; no live steering, rollback
or force-kill is implied. Allow both tool names. With subagents enabled,
...
the child turn; `Agent`/`AgentManage` return successful `awaiting_reply` with question
details. Reply via ordinary `AgentManage({id, message})` in the same Pi session.
