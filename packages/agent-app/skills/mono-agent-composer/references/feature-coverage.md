...
| Interactive setup wizard (preset/custom; exact `IDENTITY.md` → `## Role` prompt/outcome; walks name/Role → model/effort/fallbacks → optional-capabilities gate for channels/memory/observability → capability details → tools → sandbox → observability → creation review; declining the gate still runs the tool framing, the sandbox choice, and the default-No high-risk confirmation; Journal/BuJo explicitly choose Ollama or LM Studio service root/model/dimension/optional auth env using typed discovery and a real probe; macOS starts the background agent and prints the browser handoff `status` → `mono-agent web run --loopback` → `http://127.0.0.1:5050`) | cli | `mono-agent init` (no flags, on a TTY; `setup` alias); manual embedding entry still requires readiness probe; flags/non-TTY stay scaffold-only; unsupported platforms use manual configuration/foreground start plus the same browser console | — |
