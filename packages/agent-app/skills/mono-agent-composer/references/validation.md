...
| Observability | Confirm a run writes a JSONL artifact with sensitive object-key values redacted and strings capped; retained free text is not content-scanned. If an `observability.exporters` Phoenix entry is set, confirm the trace appears in Phoenix. |
