...
| Surface | Smoke |
| --- | --- |
| Web console | Start the host, connect through `mono-agent web run --loopback`, and complete one real prompt against the running responder. |
...
| Observability | Confirm a run writes a JSONL artifact with strings capped: non-numeric values under sensitive-looking object keys are redacted; numeric values under matched keys are retained; retained free text is scanned for a closed set of high-confidence credential shapes. |
...
| Detached persistent child recovery | With ProcessJobs and persistent Agent/AgentManage enabled on an exact-conversation Pi route, create a retained recovery receipt and inspect it before acknowledgement. Verify one acknowledged continuation runs once, a repeated request is rejected without another provider call, and a failed acknowledged `close:true` continuation preserves its pending AskParent question. Make the private registry unreadable in a disposable fixture and require a path-free unavailable result with the acknowledgement still usable after restoration. `mono-agent doctor` may report only bounded retained, unresolved and owner-unavailable counts, never identifiers or private paths. |
