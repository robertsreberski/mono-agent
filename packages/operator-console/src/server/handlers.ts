...
async function routeApi(
  ...
  if (method === "GET" && path === "/api/config") {
    try {
      const { json: raw, version } = await readSettingsJson(deps.configPath);
      deps.log?.({ kind: "read", path: deps.configPath });
      return json(res, 200, {
        config: redactSettingsForFieldGroups(raw, deps.fieldGroups),
        version,
      });
    } catch (error) {
...
async function handlePut(
  ...
  let parsed: { patch?: SettingsJson; expectedVersion?: string };
  try {
    parsed = JSON.parse(body) as { patch?: SettingsJson; expectedVersion?: string };
  } catch (error) {
  ...
  // disk. The server only persists keys that map to a registered FieldGroup
  ...
  const validated = validateSettingsPatch(parsed.patch, deps.fieldGroups);
  if (!validated.ok) {
    deps.log?.({
  ...
  let current;
  try {
    current = await readSettingsJson(deps.configPath);
  } catch (error) {
    return json(res, 500, errorBody(error));
  }
  ...
  try {
    const result = await writeSettingsJson({
      path: deps.configPath,
...
export const __test = { MIME_TYPES, OPERATOR_CONSOLE_STATIC_DIR_FOR_TESTS: resolve(OPERATOR_CONSOLE_STATIC_DIR) };
