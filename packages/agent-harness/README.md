...
The harness may depend on core building blocks: agent-contracts, context, observability, runtime-adapter, skills, and tool-policy. Sandbox policy/types are owned by runtime-adapter. It accepts the `MemoryStore` contract from `@mono-agent/agent-contracts` without depending on a concrete memory backend. It must not depend on communication adapters or host/demo code.
