...
function createBridge(options: {
  ...
  trigger?: ConstructorParameters<typeof WhatsAppBridge>[0]["trigger"];
} = {}): { bridge: WhatsAppBridge; socket: FakeSocket; responder: AgentResponder } {
  ...
  const bridgeOptions: WhatsAppBridgeOptions = {
    socket,
    responder,
  ...
  const bridge = new WhatsAppBridge(bridgeOptions);
...
describe("WhatsAppBridge", () => {
  it("fails closed without allowed chats or explicit allowAllChats", () => {
    ...
    expect(() => new WhatsAppBridge({ socket, responder })).toThrow(
