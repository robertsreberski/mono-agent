...
export interface WhatsAppBridgeMessages {
  welcomeText?: string;
  helpText?: string;
  busyText?: string;
...
export interface WhatsAppBridgeStreamOptions {
  initialStatusText?: string;
  sendInitialStatus?: boolean;
  maxMessageChars?: number;
}

export interface WhatsAppBridgeLogger extends WhatsAppMessageStreamLogger {
...
export interface WhatsAppBridgeOptions {
  ...
  stream?: WhatsAppBridgeStreamOptions;
  messages?: WhatsAppBridgeMessages;
  logger?: WhatsAppBridgeLogger;
}

export type WhatsAppBridgeIgnoredReason =
...
export type WhatsAppMessageHandlingResult =
  ...
  | {
      kind: "ignored";
      reason: WhatsAppBridgeIgnoredReason;
      chatJid?: WhatsAppJid;
      messageId?: string;
    }
  | {
...
const DEFAULT_MESSAGES: Required<WhatsAppBridgeMessages> = {
  welcomeText:
  ...
  unsupportedText: "I can only handle WhatsApp text messages in this bridge for now.",
  mentionRequiredText: "Mention this bot to trigger the agent in this group.",
};

const DEFAULT_STREAM_OPTIONS: Required<WhatsAppBridgeStreamOptions> = {
...
export class WhatsAppBridge {
  ...
  private readonly streamOptions: Required<WhatsAppBridgeStreamOptions>;
  private readonly messages: Required<WhatsAppBridgeMessages>;
  private readonly logger: WhatsAppBridgeLogger | undefined;
  ...
  constructor(options: WhatsAppBridgeOptions) {
    ...
    if (!this.allowAllChats && this.allowedChatJids.size === 0) {
      throw new TypeError(
        "WhatsAppBridge requires allowedChatJids or allowAllChats: true.",
      );
    }
  }
  ...
  private async respondToMessage(
    ...
    try {
      ...
      this.logger?.error?.("WhatsApp bridge responder failed.", {
...
async function finishSafely(
  stream: AgentMessageStream,
  text: string,
  logger: WhatsAppBridgeLogger | undefined,
): Promise<void> {
  try {
