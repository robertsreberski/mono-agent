import { Container, matchesKey, SelectList, Text, TuiMainScreen } from "@earendil-works/pi-tui";
...
/** Prefix for non-selectable provider group headings in the model picker. */
...
export class MonoAgentTuiApp {
  ...
  /** Bounded provider catalog used to group the model picker; absent on older agents. */
  ...
  private showHelp(): void {
    const help = new Text(
      [
        ...
        styles.dim("/quit closes only this console; it does not stop the background agent"),
        "",
  ...
  /** Build configured-first provider groups, with an Unknown group for leftovers. */
  private buildModelPickerItems(current: string | undefined): SelectItem[] {
    ...
    const providerIds = Array.from(new Set(this.availableProviders.map((provider) => provider.id)));
    ...
    for (const providerId of providerIds) {
      ...
      const models = this.availableModels.filter((model) => providerIdFor(model) === providerId);
    ...
    if (unknown.length > 0) {
      ...
      for (const model of unknown) items.push(this.modelPickerRow(model, current));
    }
    return items;
  }

  /** Build one selectable model row while preserving its canonical model ref. */
   ...
   * `onChoose` reports whether the row was a real choice; decorative provider
   * headings return false so the overlay stays open.
   ...
   */
  private openPickerOverlay(title: string, items: SelectItem[], onChoose: (item: SelectItem) => boolean): void {
    const list = new SelectList(items, 10, selectListTheme);
    list.onSelect = (item: SelectItem) => {
      if (!onChoose(item)) return;
      this.closePicker();
    };
