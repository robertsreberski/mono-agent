...
export interface SecretSafeLogSink {
...
/** Trusted adapter-owned patterns and diagnostic markers, never log input. */
export interface LogRedactorOptions {
  readonly tokenMarker: string;
  readonly unavailableMarker: string;
  readonly truncatedMarker: string;
  readonly binaryMarker: string;
  readonly tokenPatterns: readonly {
...
export interface LogRedactor {
  redactText(text: string, knownSecrets?: readonly string[]): string;
  redactErrorMessage(error: unknown, knownSecrets?: readonly string[]): string;
  redactError(error: unknown, knownSecrets: readonly string[]): Error;
  createLogger(
    logger: SecretSafeLogSink | undefined,
    knownSecrets: readonly string[],
  ): SecretSafeLogSink | undefined;
}
...
export function isSafePrototypeInstance(
  value: unknown,
  prototype: object,
): boolean {
  if (
...
export function readSafeDataProperty(value: unknown, key: string): unknown {
  if (
    value === null
    || (typeof value !== "object" && typeof value !== "function")
  ) return undefined;
...
/** Build one bounded, descriptor-only sanitizer with adapter-owned token rules. */
export function createLogRedactor(options: LogRedactorOptions): LogRedactor {
  const REDACTED_TOKEN = options.tokenMarker;
  const REDACTED_BEARER = "[REDACTED_BEARER_CREDENTIAL]";
  const REDACTION_FAILED = options.unavailableMarker;
  const REDACTION_LIMIT = options.truncatedMarker;
  const REDACTED_FRAGMENTED_CREDENTIAL = "[REDACTED_FRAGMENTED_CREDENTIAL]";
  const OMITTED_BINARY_DATA = options.binaryMarker;
  const REPEATED_REFERENCE = "[Repeated]";
  const REDACTION_BOUNDARY_MARKERS = [REDACTED_TOKEN, REDACTED_BEARER] as const;
  const PRESERVED_LOG_MARKERS = new Set([
    REDACTED_TOKEN,
    REDACTED_BEARER,
    REDACTION_FAILED,
    REDACTION_LIMIT,
  ...
  return { redactText, redactErrorMessage, redactError, createLogger };

  /** Redact configured credentials and the caller-provided token patterns. */
  function redactText(
    text: string,
    knownSecrets: readonly string[] = [],
  ): string {
    ...
    for (const replace of [
      ...options.tokenPatterns.map(({ pattern, replacement }) => (value: string) => (
        value.replace(pattern, replacement ?? REDACTED_TOKEN)
      )),
  ...
  function replaceKnownSecretBounded(text: string, secret: string): string {
    ...
    if (REDACTED_TOKEN.length > secret.length) {
      let occurrences = 0;
      let offset = 0;
      while (offset <= text.length - secret.length) {
        ...
        const projected = text.length
          + occurrences * (REDACTED_TOKEN.length - secret.length);
        if (projected > MAX_LOG_TEXT_CHARS) return REDACTION_LIMIT;
        offset = index + secret.length;
      }
    }
    const replaced = text.split(secret).join(REDACTED_TOKEN);
  ...
  function redactErrorMessage(
    ...
    try {
      if (typeof error === "string") {
        return redactText(error, knownSecrets);
      }
      if (
        error === null
        || error === undefined
        ...
        return redactText(String(error), knownSecrets);
      }
      ...
      if (typeof error === "object" || typeof error === "function") {
        ...
        return message === undefined
          ? REDACTION_FAILED
          : redactText(message, knownSecrets);
      }
    } catch {
  ...
  function createLogger(
    ...
    const wrapped: SecretSafeLogSink = {};
      ...
      wrapped[level] = (message, metadata) => {
        ...
        try {
          if (safeKnownSecrets === undefined) {
            ...
            safeMessage = redactText(message, safeKnownSecrets);
            safeMetadata = metadata === undefined
              ? undefined
              : sanitizeLogRecord(metadata, safeKnownSecrets);
          }
        } catch {
          safeMetadata = { redaction: REDACTION_FAILED };
        }
  ...
  function sanitizeLogRecord(
    ...
    const context: LogSanitizeContext = {
    ...
    const sanitized = sanitizeLogValue(record, knownSecrets, context, 0);
    if (context.fragmentedCredential) {
      const scrubbed = scrubFragmentedLogValue(sanitized);
      return isRecord(scrubbed) ? scrubbed : { value: scrubbed };
    }
    return isRecord(sanitized) ? sanitized : { value: sanitized };
  }

  interface LogSanitizeContext {
  ...
  function sanitizeLogValue(
    value: unknown,
    knownSecrets: readonly string[],
    context: LogSanitizeContext,
    depth: number,
    container?: "headers" | "query",
  ): unknown {
    if (typeof value === "string") {
      if (container !== undefined) return REDACTED_BEARER;
      return sanitizeLogText(value, knownSecrets, context);
    }
    ...
    if (
      ...
      observeLogFragment(String.fromCharCode(value), knownSecrets, context);
    }
    ...
    if (depth >= MAX_LOG_DEPTH || context.remainingNodes <= 0) {
      return Array.isArray(value) ? [REDACTION_LIMIT] : truncatedLogRecord();
    }
    ...
    try {
      ...
      if (boxedString !== undefined) {
        return container === undefined
          ? sanitizeLogText(boxedString, knownSecrets, context)
          : REDACTED_BEARER;
      }
      ...
      if (href !== undefined) {
        if (container !== undefined) return REDACTED_BEARER;
        return sanitizeLogText(href, knownSecrets, context);
      }
      if (Array.isArray(value)) {
        return sanitizeLogArray(value, knownSecrets, context, depth, container);
      }
  ...
  function copySafeOwnProperties(
    ...
    for (const key of selectedNames) {
      ...
      const safeKey = sanitizeLogText(key, knownSecrets, context, false);
      ...
      if (container !== undefined || sensitiveLogKey(key)) {
        ...
        setSafeProperty(target, safeKey, sanitizeLogValue(
          descriptor.value,
          knownSecrets,
          context,
          depth + 1,
  ...
  function safeStringProperty(value: object, key: string): string | undefined {
    const property = readSafeDataProperty(value, key);
  ...
  function sanitizeLogText(
    ...
    if (trackFragment) observeLogFragment(text, knownSecrets, context);
    if (text.length > context.remainingOutputChars) return REDACTION_LIMIT;
    const safe = redactText(text, knownSecrets);
  ...
  function observeLogFragment(
    ...
    const safeText = redactText(text, knownSecrets);
    ...
    if (
      ...
      && prefix.length > 0
      && redactText(combined, knownSecrets) !== combined
    ) {
   ...
   */
  function scrubFragmentedLogValue(value: unknown): unknown {
    if (typeof value === "string") {
      return PRESERVED_LOG_MARKERS.has(value)
        ? value
        : REDACTED_FRAGMENTED_CREDENTIAL;
    }
    if (Array.isArray(value)) {
      ...
      for (let index = 0; index < value.length; index += 1) {
        ...
        if (descriptor !== undefined && "value" in descriptor) {
          safe[index] = scrubFragmentedLogValue(descriptor.value);
        }
      }
      return safe;
    }
    ...
    for (const key of Object.getOwnPropertyNames(value)) {
      const descriptor = Object.getOwnPropertyDescriptor(value, key);
      if (descriptor !== undefined && "value" in descriptor) {
        setSafeProperty(safe, key, scrubFragmentedLogValue(descriptor.value));
      }
    }
    return safe;
  }

  function truncatedLogRecord(): Record<string, unknown> {
    const safe = Object.create(null) as Record<string, unknown>;
  ...
  function sanitizeLogArray(
    ...
    if (
      ...
      && redactText(fragmented, knownSecrets) !== fragmented
    ) {
    ...
    if (container === undefined && first !== undefined && sensitiveLogKey(first)) {
      return [sanitizeLogText(first, knownSecrets, context), REDACTED_BEARER];
    }
    ...
    for (let index = 0; index < length; index += 1) {
      ...
      safe[index] = sanitizeLogValue(entry, knownSecrets, context, depth + 1);
    }
    return safe;
  }
  ...
  function sanitizeSecretContainerArray(
    ...
    for (let index = 0; index < length; index += 1) {
      ...
      if (Array.isArray(entry)) {
        safe[index] = sanitizeLogValue(entry, knownSecrets, context, depth + 1, container);
        continue;
      }
  ...
  /** Return an Error safe for callbacks that may log or serialize it. */
  function redactError(error: unknown, knownSecrets: readonly string[]): Error {
    try {
      const record = sanitizeLogRecord(error, knownSecrets);
      const safe = new Error(
        typeof record.message === "string"
          ? record.message
          : redactErrorMessage(error, knownSecrets),
      );
      const name = readSafeDataProperty(error, "name");
      if (typeof name === "string") safe.name = redactText(name, knownSecrets);
