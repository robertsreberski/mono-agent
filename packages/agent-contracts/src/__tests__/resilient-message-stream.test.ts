...
describe("ResilientMessageStream subagent activity", () => {
  it.each([false, true])("preserves AgentManage identity through streamed start and completion (close=%s)", async (close) => {
    const transport = new FakeTransport({ maxMessageChars: 500 });
    const stream = makeStream(transport, { finalOnly: true, showHints: true });
    await stream.event({ type: "tool_call_started", id: "send-1", name: "AgentManage", arguments: { id: "critic-7", ...(close ? { close: true } : { message: "continue" }) } });
    ...
    await stream.event({ type: "tool_call_completed", id: "send-1", name: "AgentManage", executionMs: 10 });
  ...
  it("preserves namespace-qualified AgentManage identity and close semantics", async () => {
    const transport = new FakeTransport({ maxMessageChars: 500 });
    const stream = makeStream(transport, { finalOnly: true, showHints: true });
    const name = "mcp__helper__AgentManage";
