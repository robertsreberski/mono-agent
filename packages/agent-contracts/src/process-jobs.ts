...
/** Stable secret-free messages paired with public process-job error codes. */
export const PROCESS_JOB_PUBLIC_ERROR_MESSAGES: Readonly<Record<ProcessJobErrorCode, string>> = Object.freeze({
...
export function processJobPublicError(code: ProcessJobErrorCode): ProcessJobProjectionError {
...
export function parseProcessJobProjection(value: unknown): ProcessJobProjection {
  ...
  const parsed = value as unknown as ProcessJobProjection;
  return parsed.lastError === null
    || parsed.lastError.message === PROCESS_JOB_PUBLIC_ERROR_MESSAGES[parsed.lastError.code]
    ? parsed
    : { ...parsed, lastError: processJobPublicError(parsed.lastError.code) };
}
