...
It composes `@worklab-ai/memory-journal` and `@worklab-ai/memory-graph` and
...
consolidation). Semantic search is layered on later via `@worklab-ai/memory-search`.

## Install / Usage

```bash
pnpm --filter @worklab-ai/memory-mcp run build
```
...
```ts
import { createMemoryMcpServerFromConfig } from "@worklab-ai/memory-mcp";
...
`@worklab-ai/memory-journal` and `@worklab-ai/memory-graph` context packages for
...
consolidation, or build prompt context. Those belong to `@worklab-ai/memory-search`,
the consolidation cron job, and `@worklab-ai/context`.

## Verification

```bash
pnpm --filter @worklab-ai/memory-mcp run build
pnpm --filter @worklab-ai/memory-mcp run typecheck
pnpm --filter @worklab-ai/memory-mcp run test
```
