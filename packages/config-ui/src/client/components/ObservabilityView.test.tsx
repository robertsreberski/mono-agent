...
describe("<ObservabilityView/>", () => {
  it("shows a disabled state instead of fixture data when the bridge has no observability", async () => {
    const fetchObservedRuns = vi.fn<ConfigUiClient["fetchObservedRuns"]>().mockResolvedValue({
      enabled: false,
      runs: [],
      warnings: ["Observability is not configured for this bridge."],
    } satisfies ObservabilityRunsResponse);
    const fetchObservedRun = vi.fn<ConfigUiClient["fetchObservedRun"]>();
  ...
  it("lists recorded runs and loads an expandable event timeline", async () => {
    ...
    const fetchObservedRun = vi.fn<ConfigUiClient["fetchObservedRun"]>().mockResolvedValue({
      enabled: true,
      artifactDir: "/repo/.mono-agent/artifacts",
      run: runDetail,
