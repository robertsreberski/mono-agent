...
type ViewName = "config" | "observability";
    ...
    <main className="mx-auto flex min-h-screen w-full max-w-6xl flex-col gap-5 px-4 py-6 pb-[calc(5rem+env(safe-area-inset-bottom))] sm:gap-6 sm:px-6 sm:py-10 lg:px-8">
      <header className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto] md:items-start">
        <div className="min-w-0 space-y-1.5">
          <h1 className="text-2xl font-medium tracking-tight sm:text-3xl">Mono Agent — Config</h1>
        ...
        <nav className="flex min-w-0 gap-2 overflow-x-auto rounded-xl bg-muted/60 p-1" aria-label="Primary views">
          <NavButton active={view === "config"} onClick={() => setView("config")}>Config</NavButton>
          <NavButton active={view === "observability"} onClick={() => setView("observability")}>Observability</NavButton>
        </nav>
      </header>
      <Separator />
      <div className="min-w-0">
        {view === "config" ? <ConfigForm client={client} /> : <ObservabilityView client={client} />}
      </div>
    </main>
  );
}
...
function viewFromHash(hash: string): ViewName {
  return hash === "#observability" ? "observability" : "config";
}
