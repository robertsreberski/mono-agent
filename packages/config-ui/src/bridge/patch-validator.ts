 ...
 *
 * Every leaf path in the incoming patch MUST correspond to a registered
 * FieldDefinition. Unknown leaves are rejected — the bridge never persists
 * arbitrary keys. Per-leaf coercion runs the value through the field's
 * declared kind so invalid integers / shapes are surfaced before the
 * file ever changes.
 *
 * The returned `patch` is a fresh object containing only the validated
 * leaves; nothing from the input survives unchecked.
 */
export function validatePatch(
  ...
  return { ok: true, patch: cleanPatch as MonoAgentConfigJson };
}
...
interface CoerceOk {
  readonly ok: true;
  /** undefined means "delete this leaf"; anything else gets persisted. */
  readonly value: unknown;
}
...
function coerceLeaf(field: FieldDefinition, raw: unknown): CoerceResult {
  // Treat null / empty-string as "clear this field" — patch deletes the leaf.
  if (raw === null) {
