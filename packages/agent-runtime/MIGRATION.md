...
The runtime exact-pins Pi AI and Pi Agent Core at `0.87.1` (the terminal TUI
...
`@earendil-works/pi-agent-core` are both `0.87.1`** (the initial Pi 0.80
...
Packed npm consumers resolve the runtime-owned exact Pi AI 0.87.1 copy for both
the runtime and Agent Core's `^0.87.1` dependency. The release guard verifies
...
2. **Remove direct provider ownership.** Delete Worklab production imports from
  ...
  to the exact Pi AI `0.87.1` and Pi Agent Core `0.87.1` compatibility pins
...
3. **Use the public Pi surfaces.** Run models through
  ...
  exact Pi AI `0.87.1` and Pi Agent Core `0.87.1` compatibility pins inside the runtime. OAuth login adapters
