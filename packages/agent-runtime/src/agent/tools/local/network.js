// @ts-check
// Native, host-governed transport. No upstream impersonation/proxy/retry code.
...
export const LOCAL_USER_AGENT = "mono-agent-local/1 (web research)";

export function assertLocalTarget(url, options) {
 ...
 */
export async function localRequest(url, options, init = {}, maxBytes = 2 * 1024 * 1024) {
  ...
  const result = await guardedSearch(kind, key, {
    ...
    try {
      ...
      const response = await options.fetchImpl(target.href, {
        ...
        headers: { Accept: "text/html,text/plain;q=0.9", "User-Agent": options.userAgent ?? LOCAL_USER_AGENT, ...init.headers },
      });
      const bytes = await readLocalBytes(response, maxBytes, options.signal);
      ...
      if (response.status === 429 || challenge || softGate) {
        const limited = rateLimited({ backend: kind, label: "Local engine" }, "access refused", response);
  ...
  if (!result.ok) throw Object.assign(new Error("Local request refused."), {
...
export async function readLocalBytes(response, maxBytes, signal) {
  if (Number(response.headers.get("content-length")) > maxBytes) {
    await response.body?.cancel();
    throw Object.assign(new Error("Local response exceeded byte limit."), { code: "response_too_large" });
  }
  ...
  try {
    for (;;) {
      ...
      if (size > maxBytes) throw Object.assign(new Error("Local response exceeded byte limit."), { code: "response_too_large" });
