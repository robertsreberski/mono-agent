...
describe("extractEntities", () => {
  ...
  it("parses well-formed entities and relations from canned LLM response", async () => {
    ...
    const result = await extractEntities("Morgan maintains mono-agent", llm);
    expect(result.entities).toHaveLength(2);
    expect(result.entities[0]).toMatchObject({ id: "person:morgan", name: "Morgan", type: "person" });
    expect(result.entities[1]).toMatchObject({ id: "project:mono-agent", name: "mono-agent" });
    expect(result.relations).toHaveLength(1);
    expect(result.relations[0]).toMatchObject({ src: "person:morgan", dst: "project:mono-agent", relation: "maintains" });
  });
