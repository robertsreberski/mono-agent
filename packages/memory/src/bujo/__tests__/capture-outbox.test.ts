...
describe("capture outbox", () => {
  ...
  it("converges instead of throwing when a pre-existing entity row diverges from the canonical record", () => {
    const root = tempRoot();
    const item = bullet("ROBIN-ADD", "Robin owns the capture that used to wedge the outbox.");
    const file = relative(root, dailyFilePath(root, NOW));
    writeCaptureIntent(root, [{
      candidateIndex: 0,
      kind: "add",
      ...
      entities: [{ id: "person:robin", name: "Robin", type: "person", createdAt: NOW.toISOString() }],
    }, NOW.toISOString());
    const db = openMemoryDb({
      path: join(root, "memory.db"),
    ...
    try {
      ...
      expect(db.getEntity("person:robin")).toEqual({
        id: "person:robin",
        name: "Robin",
        type: "person",
        createdAt: NOW.toISOString(),
  ...
  it("converges when graph.jsonl and the index disagree on createdAt/summary", () => {
    ...
    const item = bullet("ROBIN-GRAPH", "Robin's capture reconciles against the canonical graph line.");
    const file = relative(root, dailyFilePath(root, NOW));
    writeCaptureIntent(root, [{
      candidateIndex: 0,
      kind: "add",
  ...
  it("removes stale derived associations when canonical capture evidence supersedes them", () => {
    const root = tempRoot();
    const item = bullet("DERIVED-SUPPRESSED", "Morgan and Robin are both named here.");
