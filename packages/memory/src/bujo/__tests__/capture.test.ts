...
describe("captureTurn", () => {
  ...
  it("does not append duplicate graph records across repeated captures", async () => {
    ...
    const llm = fakeLlm([
      [
        "Extract one bounded",
        JSON.stringify({
          memories: [{ type: "note", text: "Robin prefers quiet mornings", salience: 0.7, isInsight: false, entityIds: ["person:robin"] }],
  ...
  it("replays graph evidence only after exact ADD/UPDATE/SUPERSEDE/NOOP outcomes match", async () => {
    ...
    await seed(db, root, "NOOP", "Robin prefers quiet mornings.");
        ...
        if (options?.label === "capture:extract") {
          return JSON.stringify({
            memories: [
              ...
              { type: "note", text: "Robin prefers quiet mornings.", salience: 0.7, isInsight: false, entityIds: ["person:robin"] },
            ],
            entities: [
              { id: "project:aster", name: "Aster", type: "project" },
              { id: "person:morgan", name: "Morgan", type: "person" },
              { id: "project:atlas", name: "Atlas", type: "project" },
              { id: "person:robin", name: "Robin", type: "person" },
            ],
            relations: [],
          });
        }
    ...
    expect(db.get("NOOP")?.text).toBe("Robin prefers quiet mornings.");
    expect(readdirSync(join(root, ".capture-outbox"))).toHaveLength(1);
