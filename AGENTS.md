...
- Optional plugin-tier extras live under `extras/<package-name>/` when cataloged with `publishable: true` and `tier: "plugin"` (published in the npm lockstep but outside the core `@mono-agent/agent-app` closure, loaded via `channels.plugins[]`, as a request-scoped runtime extension, through an explicitly selected plugin backend, or as an explicitly paired companion MCP server); the current extras are `@mono-agent/a2a-adapter`, `@mono-agent/agent-orchestrator`, `@mono-agent/docs-mcp`, `@mono-agent/memory-supermemory`, `@mono-agent/observability-phoenix`, and `@mono-agent/whatsapp-adapter`.
