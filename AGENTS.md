...
- Optional plugin-tier extras live under `extras/<package-name>/` when cataloged with `publishable: true` and `tier: "plugin"` (published in the npm lockstep but outside the core `@mono-agent/agent-app` closure, loaded via `channels.plugins[]`, as a request-scoped runtime extension, or through an explicitly selected plugin backend); the current extras are `@mono-agent/a2a-adapter`, `@mono-agent/agent-orchestrator`, `@mono-agent/memory-supermemory`, and `@mono-agent/whatsapp-adapter`.
