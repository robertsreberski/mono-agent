...
Current catalog: 17 core-tier packages, 5 plugin-tier extras, and 1 unscoped alias; all 23 publishable entries release in lockstep.
...
flowchart TB
  ...
  subgraph P_category_operator_surface["operator-surface"]
    P_mono_agent_session_web["@mono-agent/session-web"]
  ...
  P_mono_agent_session_web --> P_mono_agent_agent_contracts
  P_mono_agent_session_web --> P_mono_agent_observability
...
| `@mono-agent/session-web` | `@mono-agent/agent-contracts`, `@mono-agent/observability` |
...
- TUI and web call the operator adapter's bidirectional endpoint; session-web and other read-only consumers use its live event relay and observability artifacts.
...
| `@mono-agent/session-web` | `core` / `operator-surface` | Maintains the legacy read-only Session Recorder PWA for programmatic embedders, combining discovered agents' recorded and live runs while retirement remains pending. | [README for @mono-agent/session-web](./packages/session-web/README.md) · [npm for @mono-agent/session-web](https://www.npmjs.com/package/@mono-agent/session-web) |
...
| `@mono-agent/operator-adapter` | `core` / `communication` | Exposes local operator endpoints: structured TUI NDJSON turns and read-only live SSE run-event streams. | [README for @mono-agent/operator-adapter](./packages/operator-adapter/README.md) · [npm for @mono-agent/operator-adapter](https://www.npmjs.com/package/@mono-agent/operator-adapter) |
