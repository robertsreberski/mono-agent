...
| `tools.disallowedTools` | string[] | Denylist; deny always wins, even under allow-all. Filters built-ins, `ReadSkill`, `RunHistory`, and adapter send tools; the host no-tools `"*"` form denies all of them. On the pi-native runtime it does **not** filter external MCP-server tools (see below). |
