...
All three are configured in `mono-agent.config.json`; every route runs the Pi runtime, so enforcement is uniform and unsupported combinations fail closed. The tool policy is **allow-all by default**. When `sandbox.mode` is `native`, SRT confines what tool subprocesses may reach on disk and over the network. When no native sandbox is active, telemetry reports SRT as `disabled` and Bash/stdio MCP subprocesses run unsandboxed; the default `sandbox.fallback: "fail-closed"` fails the run instead when a native engine cannot be acquired, and an unsafe fallback is loudly warned. Unsupported capabilities are never silently removed.
