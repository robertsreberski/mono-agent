...
Then open `http://127.0.0.1:5050`, choose the agent, and start a conversation. `web run` binds the console's HTTP listener to this computer and configures no proxy route, but it does not remove one that already exists — an earlier managed start, a Serve handler, or your own proxy can still make it reachable elsewhere, so check `tailscale serve status` or your proxy before treating the console as local-only. The managed `mono-agent web start` service runs in the background and, on macOS, claims its own Tailscale Serve HTTPS route, so `--loopback` alone does not make a managed console local; neither mode has an application login. [Getting Started → Quickstart](/getting-started/quickstart/) explains every branch of that path, including what a bare `init` proves before it calls the agent ready, and [Install & prerequisites](/getting-started/install/) covers both console modes, pinned installs, one-shot scaffolding, and source builds.
