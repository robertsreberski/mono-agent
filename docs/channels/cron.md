...
**Fail-open is absolute.** A non-zero exit, a signal, a missing executable, a timeout, output over the caps, or a malformed verdict runs the job with its plain prompt and records a stable error code (`exit_nonzero`, `signal`, `spawn_failed`, `timeout`, `invalid_json`, `invalid_verdict`, `output_overflow`) — never a silent stop. The durable record carries codes and a bounded reason only: raw gate stdout, stderr, argv, and environment values are never persisted, and a failed gate's stderr may appear once in the log, truncated to 1 KiB at `warn`.
