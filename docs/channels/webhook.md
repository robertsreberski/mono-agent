...
| `model` | string | `runtime.model` | Per-endpoint model override (e.g. `anthropic:claude-opus-4-8`). A request body `model` wins. See [Per-trigger model & effort](#per-trigger-model--effort). |
