...
| Key | Required | Notes |
| --- | --- | --- |
| `name` | yes when no root name is available | Human-readable Agent Card name. Defaults from root `agent.name`; plugin `config.agent.name` / `MONO_AGENT_A2A_AGENT_NAME` wins. |
...
| `MONO_AGENT_A2A_AGENT_NAME` | plugin `config.agent.name` (wins over root `agent.name`) |
