...
Telegram and Slack default to delivering **only the final answer** — answer tokens are not streamed into the chat. An inbound turn starts with the channel's lightweight working indicator. If the agent starts a tool, one temporary message exposes a cumulative, user-safe activity ledger such as `🌐 Browsing https://example.com` or `🖥️ Running pnpm test`. Later tool starts edit that same message. Confirmed consumed live guidance adds `↪️ Steered: “<safe preview>”`. At completion, both adapters post the answer as a fresh message and then best-effort delete the ledger. Reasoning and answer deltas never overwrite the ledger while work is in progress. This is built-in adapter behavior, not a JSON field you set in `mono-agent.config.json`.
...
When a shell tool call carries a `description`, that intent is shown instead of the command line, without the `Running` verb: `🖥️ Show working tree status`. The field arrives either because the runtime declares it (the Claude Agent SDK's shell tool does) or because the model volunteers it (activity events carry the raw tool input, so an undeclared key still survives). Calls without one fall back to the command, so a ledger can mix both forms when only some turns supply a description.
...
🤖 Starting agent "researcher"
  ↳ 📖 Reading src/router.js
  ↳ 🔎 Searching files for createTool (×2)
🤖 Starting agent "reviewer"
  ↳ 📖 Reading src/agent-tool.js
```
...
Consumed live input uses the same normalization and redaction boundary. Its
completed synthetic tool lifecycle contains only the safe preview and
`Consumed by current run`; the full follow-up remains the human message and is
...
Only proved non-delivery is eligible for the ordinary queued-turn fallback.
Once native delivery may have happened, an ambiguous result is not retried;
this prevents a duplicate turn at the cost of requiring a deliberate resend.
