...
| --- | --- | --- | --- |
| Telegram | Bot long polling | `telegram` | [Telegram](telegram.md) |
| Slack | Socket Mode bot | `slack` | [Slack](slack.md) |
| WhatsApp | Baileys socket (QR login) | `whatsapp` | [WhatsApp](whatsapp.md) |
| Webhook | HTTP POST, sync or async | `webhook` | [Webhook](webhook.md) |
| OpenAI-compatible API | `/v1/chat/completions` (SSE) | `openaiApi` | [OpenAI-compatible API](openai-api.md) |
| A2A | Agent-to-Agent provider/consumer | `a2a` | [A2A](a2a.md) |
| Cron | Scheduled prompts | `cron` | [Cron](cron.md) |
...
| You want… | Use | Why |
| --- | --- | --- |
| A human chatting interactively | [Telegram](telegram.md), [Slack](slack.md), or [WhatsApp](whatsapp.md) | Conversational adapters with allowlists, working indicators, and final-answer delivery |
| Programmatic / pipeline invocation | [Webhook](webhook.md) or [A2A](a2a.md) | Webhook for plain HTTP POST (sync or async polling); A2A for agent-to-agent calls with Agent Card discovery |
| A chat UI (e.g. Open WebUI) | [OpenAI-compatible API](openai-api.md) | Exposes `/v1/models` + `/v1/chat/completions` with token-by-token SSE streaming |
| Scheduled / unattended runs | [Cron](cron.md) | Timezone-aware five-field jobs that invoke the responder on a schedule |
...
Replies go back over the same channel that received the request. To send *outbound* messages — proactive notifications from cron/webhook turns, or app-owned send tools like `slack_send_message` and `telegram_send_message` — see [Delivery & send tools](delivery-and-send-tools.md). Note that these send tools require the target adapter to already be enabled and configured, and the adapter's own allowlist remains the delivery boundary.

## Custom transports

The built-in channels are config-driven. For a bespoke transport, implement a `ChannelDriver` and pass it via `startMonoAgentApp({ drivers })` — this is **code-only**. See [Custom channels](../programmatic/custom-channels.md).
