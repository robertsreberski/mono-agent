...
| Recalled memory (optional) | memory subsystem | config | see [Memory](../memory/capture-and-recall.md) |
...
Paths are resolved relative to the agent folder. `mono-agent init` scaffolds an `IDENTITY.md` for you; see [Folder Layout](../config/folder-layout.md).
...
- **Identity & soul** are plain markdown loaded into every prompt. Identity carries role and boundaries; soul is an optional secondary voice/guardrail document. See [Identity & Soul](identity-and-soul.md).
...
- **Selected skills** are loaded explicitly and each is capped at `context.skillMaxBytes` bytes (default 48000, valid range 256–1,000,000). See [Skills](skills.md).
...
- [Identity & Soul](identity-and-soul.md) — authoring `IDENTITY.md` and the optional `SOUL.md`.
- [Skills](skills.md) — the `SKILL.md` format, `selectedSkills`, and per-skill byte caps.
- [Assembly](assembly.md) — the full prompt-assembly order and how the layers combine each turn.

## Related

- [Memory](../memory/capture-and-recall.md) — recalled memory is injected alongside context.
- [Tools](../tools/index.md) — tool definitions and policy live next to context in each turn.
