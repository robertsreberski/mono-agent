...
Arguments and results are redacted and stripped of host paths before storage, individually bounded to 8 KiB and 16 KiB, and carry original/retained byte counts plus truncation flags. A filesystem-shaped span becomes an opaque host-root token with at most two non-sensitive trailing components; surrounding commands, diagnostics, nested structure, and ordinary web URLs remain inspectable, while absolute roots, account/home prefixes, artifact roots, and private run paths do not. Artifact paths remain host-only; only non-symlink regular files below the configured run-specific `tool-output` root can become opaque references or be checked for availability. Model and clients receive only those ids and availability, and the sidecar never owns or prolongs the artifact file's lifetime. Completed calls have independent retention of 100,000 calls, 365 days, and 256 MiB of retained payload; tombstones retain bounded lookup evidence for up to 10,000 removed records and 30 days. Isolated/proactive runs are persisted but omitted from default projection and search. Only the parent `Agent` lifecycle is stored; nested child activity remains nested UI/run telemetry.

On a cold or stateless reseed, at most 32 recent completed calls are projected as path-opaque, neutralized, explicitly untrusted text bounded to 64 KiB before the current user message. The current run is always excluded, including a resume-error retry that rebuilds context after tools already ran. Fresh/stateless runs place the projection in the history prompt section. A cold history-coordinated Pi reopen carries it as a prior assistant message beside canonical structured history, so create-on-miss seeds it while a true native resume skips it with the other prior messages. A confirmed warm provider session omits it. Message compaction changes only the active context projection: retained sidecar records remain discoverable through [`SessionHistory`](/tools/mcp/#sessionhistory-retained-tool-lifecycles). A tool record may therefore exist even when cancellation, timeout, signal, interruption, or another failure produced no canonical message-history append.
...
An ordinary service restart keeps message and tool-lifecycle history plus ACP session authorizations. The explicit `mono-agent restart --clear-sessions` reset clears those stores together with provider transcripts, but does not delete `MemoryRecall`'s long-term-memory store or recorded run artifacts.
