...
An admitted, non-isolated interactive turn that settles as `cancelled` publishes a derived continuity account instead of the ordinary successful-turn commit. The account contains the original user request and one host-authored assistant history message with the partial assistant text, newest whole completed tool call/result pairs that fit, a list of work still in flight, explicit omission counts, and the real typed cancellation reason. It is bounded to 48 KiB in total and uses the same path and secret redaction as tool history. The collector retains an 8 KiB UTF-8-safe assistant prefix incrementally on every run and records omitted bytes/events, so successful or faulty streams cannot create an unbounded duplicate buffer. The fixed host notice distinguishes an operator stop, shutdown, stale-session eviction, signal, timeout-style cancellation, a recorded generic reason, and an unrecorded reason; it never incorporates runtime/provider text or presents the partial answer as complete. Bounded runtime/provider detail is named `untrustedDetail` and appears only inside tag-safe JSON preceded by an explicit untrusted-evidence warning. A per-conversation publication barrier makes the next turn wait for this account, while the cancelled provider epoch is retired so canonical history and provider-side state cannot diverge. Failed/interrupted turns, isolated proactive/continuation turns, and queued requests that never start keep their existing behavior.
