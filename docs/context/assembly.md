...
An ordinary service restart keeps active conversation history and ACP session authorizations. The explicit `mono-agent restart --clear-sessions` reset clears both together with provider transcripts, but does not delete `MemoryRecall`'s long-term-memory store or recorded run artifacts.
