...
An admitted, non-isolated interactive turn that settles as `cancelled` publishes a derived continuity account instead of the ordinary successful-turn commit. The account contains the original user request and one host-authored assistant history message with the partial assistant text, newest whole completed tool call/result pairs that fit, a list of work still in flight, explicit omission counts, and the real typed cancellation reason. It is bounded to 48 KiB in total, with at most 8 KiB of partial assistant text, and uses the same path and secret redaction as tool history. The notice distinguishes an operator stop, shutdown, stale-session eviction, signal, timeout-style cancellation, a recorded generic reason, and an unrecorded reason; it never presents the partial answer as complete. A per-conversation publication barrier makes the next turn wait for this account, while the cancelled provider epoch is retired so canonical history and provider-side state cannot diverge. Failed/interrupted turns, isolated proactive/continuation turns, and queued requests that never start keep their existing behavior.
