...
Secrets, new external MCP servers/plugins/channels, broader tool authority, and weaker sandbox/network policy are refused here and handed to the existing explicit setup flow. Approved changes retain a local rollback change id, reload after the response settles, and start a fresh provider conversation.
