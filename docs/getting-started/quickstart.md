...
Secrets, model/provider or runtime-permission changes, external MCP servers/plugins, channels and cron/proactive jobs, exporters/external memory backends, broader tool authority, and weaker sandbox/network policy are refused here and handed to the existing explicit setup flow. Approved changes run under an owner-only transaction lock, recheck the current config and Role at the commit boundary, reject symlink-parent escapes, retain a local rollback change id, reload after the response settles, and start a fresh provider conversation.
