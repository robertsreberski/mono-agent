...
- **Allow-all tools, runtime-specific narrowing.** Omit `tools.allowedTools` (or include `"*"` anywhere in it) and the agent can call every built-in available on its route (the managed `Read`, `Write`, `Edit`, `Glob`, `Grep`, `Bash`, `NodeRepl`, `WebFetch`, `WebSearch` set on the Pi bridge) and every enabled channel's send tools. Pi and supported CLI runtimes can narrow their respective surfaces; direct `codex:*` normal runs accept only effective allow-all with an empty denylist and reject restrictive policies instead of widening them.
