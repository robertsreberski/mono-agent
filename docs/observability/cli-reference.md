...
On an interactive terminal with **no flags**, `init` launches the **step-by-step wizard**: pick a preset or "custom", then answer the same model, effort, channels (multiselect), memory, **tools**, sandbox, and observability questions either way. The model step defaults to direct `codex:gpt-5.6-terra` and discovers `pi:openai-codex:gpt-5.6-terra`, OpenCode-Go, Ollama, and LM Studio candidates best-effort with short timeouts. Missing Pi auth leaves the Pi Terra candidate selectable as setup-required instead of skipping it. Webhook is selected as the functional local default; optional external plugins are not presented as installed channels. The optional fallback step reuses the discovered choices, excludes the selected primary and prior fallbacks, and adds backups one at a time. Required selected-capability secrets are masked, retained only for the run, and merged into `.env` mode `0600`, never config JSON or output. The review shows the supported provider setup plan and can run auth/preflight commands before files are written, including a secure staged Pi OAuth login or saving an OpenCode-Go API key into the Pi auth store. Before a target scaffold is written, `init` runs the selected primary model directly once in a disposable no-tool, no-MCP, one-turn runtime with an abort timeout; fallback models cannot mask that result. A setup/probe failure offers retry provider setup, change model, explicit incomplete save, or cancellation. After a successful probe it validates the folder and offers to start immediately (background service on macOS; attached foreground worker elsewhere). With `--yes` or **any** flag (`--preset`, `--model`, `--with`, `--memory`, `--fallback-models`, `--effort`, `--auth`, `--dry-run`), or when stdin is not a TTY, `init` skips the wizard and writes the default/preset scaffold non-interactively. `mono-agent setup` is an alias of `init`.
...
- A provider **absent** from both the auth store and `models.json` → `waiting`, with a `[WARN]` line and a `mono-agent auth login <provider>` hint.
