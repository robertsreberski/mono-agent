...
| `--allow-non-loopback` | Permit a non-loopback bind. `/api/*` and `/api/stream` require a bearer token: `MONO_AGENT_WEB_AUTH_TOKEN` when configured, otherwise a newly generated token. |
| `--show-auth-url` | Reveal a configured token in the printed URLs. This opt-in works only when stdout is an interactive terminal; redirected/service output stays redacted. Generated one-run tokens retain their existing printed tokenized URLs. |
