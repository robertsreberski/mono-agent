...
- **operator** — the two local operator transports are separated from communication channels. The stable `tui` metadata id is shown as `gui` with `TUI + Web` and its discovered `/gui` URL; `live` is labelled `read-only events`. JSON output keeps the original `tui` and `live` ids unchanged.
