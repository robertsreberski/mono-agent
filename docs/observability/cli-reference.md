...
| `restart [--host <addr> \| --loopback] [--port <n>] [--theme <name>] [--name <label>]` | Restart with the stored bind/theme/name or replace supplied values. macOS republishes both plists from one fresh composite main-plist identity; Linux republishes its unit and accepts host-only changes on the owned active port. |
