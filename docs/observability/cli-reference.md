...
The generated config matches [the config blueprint](/config/blueprint/). See [Provider configuration](/runtime/providers/) for the model reference grammar, [Fallback](/runtime/fallback/) for the chain, [Capture & recall](/memory/capture-and-recall/) for the memory tiers, and [Presets & capability modules](/reference/presets/) for the wizard's tools step and the no-tools guardrail.
...
- If no authenticated model is referenced, the section reports `disabled`. Read-only credential detection (Pi auth store + declared `apiKeyEnv` variables) never labels a credential as `verified`; only the exact guided route call records `verified`.
...
mono-agent memory export --bundle /path/to/bundle --json
mono-agent memory export --bundle /path/to/bundle --include-extras --allow-pending --json
mono-agent memory import prepare --bundle /path/to/bundle --plan ./import-plan.json --json
mono-agent memory import apply --plan ./import-plan.json --json
mono-agent memory import restore --backup /path/from/apply --json
```
...
`start`, `restart`, `stop`, `status`, and `logs` manage the dedicated launchd service on macOS or systemd user service on Linux. When Linux has no usable user manager, the read-only bare/status view falls back to foreground-state reporting; mutations still fail and `run` remains available. `run` is the blocking foreground path on every supported platform. A fresh CLI install binds `127.0.0.1:5050`; `--host <addr>` (for example `0.0.0.0`) explicitly widens the bind, `--loopback` selects `127.0.0.1` and conflicts with `--host`, and an existing managed service keeps the bind/port/theme/name it was published with (a stopped install's values are recovered from its LaunchAgent definition or systemd unit; an unreadable definition fails closed before any change). On macOS, `--share-tailnet` explicitly creates a mono-agent-owned Tailscale Serve HTTPS route for managed `start`/`restart`; an existing exact owned route is re-verified or migrated on restart without the flag, a provably absent ownership record is cleared but never turned into a new route, and every explicit sharing failure exits nonzero and reports whether the local console stayed up or the replacement worker was rolled back, never claiming a route was created or absent when cleanup is unproven. `run` never manages a Serve route, Linux HTTPS routes are externally managed, and no mode removes a Serve handler, reverse proxy, or tunnel it does not own. `--theme` accepts `evergreen` (default), `ocean`, `plum`, or `terracotta` on `start`, `restart`, and `run`. `--name` sets the console label used for the installed PWA name/short name, browser title, and rail brand on the same three subcommands; it defaults to the machine hostname. Pass `--name -` to clear a stored override and restore that default. `mono-agent web status --json` prints the listener, service state, and owned Tailscale route as separate fields (other proxies and routes are not inspected).

| Command / flag | Effect |
| --- | --- |
| `start [--host <addr> \| --loopback] [--port <n>] [--theme <name>] [--name <label>] [--share-tailnet]` | Install/start the paired managed macOS worker and maintenance helper or the Linux systemd user unit. A pristine install defaults to `127.0.0.1:5050`, `evergreen`, and the machine hostname; a stopped install recovers its published bind/port/theme/name from the installed definition, or fails closed before any change. `--share-tailnet` is macOS-only and creates the owned Tailscale route. |
| `restart [--host <addr> \| --loopback] [--port <n>] [--theme <name>] [--name <label>] [--share-tailnet]` | Restart with the stored bind/theme/name, replace supplied values, or use `--name -` to return to the hostname default. macOS republishes both plists from one fresh composite main-plist identity and re-verifies/migrates an existing owned Tailscale route; Linux republishes its unit and accepts host-only changes on the owned active port. |
...
| `status [--json]` | Print process health, the listener, and the mono-agent-owned Tailscale route as separate facts (other proxies/routes are not inspected). macOS also reports local/LAN/Tailscale URLs and durable log maintenance. Linux reports systemd state and externally managed HTTPS; without a user manager it falls back to the foreground-state view. `--json` emits the same fields as an object. |
...
There is no application authentication. Anyone who can reach the listener can read conversations, upload files, and operate discovered agents, so a fresh install binds `127.0.0.1` and widening is explicit (`--host`). Neither the bind nor the absence of a mono-agent-owned route proves local-only access: other proxies, tunnels, and routes are not inspected. Host/Origin checks and the absence of CORS do not turn an untrusted network into a safe one.

With `--share-tailnet`, managed start creates a conflict-free Tailscale Serve HTTPS endpoint: it uses `:443` only when free, otherwise the first free port in `8443`–`8499`, never resets another handler, records ownership, and removes only its own route. An existing exact owned route is re-verified (and migrated to a changed target) on restart without the flag; a record whose handler is provably absent is cleared but never replaced without `--share-tailnet`. Every failed explicit share exits nonzero with the local console state stated precisely: pre-mutation failures and successful rollbacks keep the healthy console running and report no route changed or the new handler removed, while a migrated-route failure rolls the replacement worker back, and an unproven cleanup reports the attempted port/target for manual inspection instead of claiming absence. If a restart changes the app port but cannot migrate an already-owned route, mono-agent restores the prior worker, service record, plist, and exact Serve route, then exits nonzero instead of leaving a healthy-looking split configuration.
