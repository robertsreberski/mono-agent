...
| replay | `f3` | Recorded runs read straight from the agent's artifact dir — runs from any channel (telegram, cron, webhook, …) expand into the key-redacted, bounded events that reached their JSONL files: thinking, tools, telemetry, failover history, error detail, plus usage and cost from the summary. Payload tails capped before persistence and RAM-buffered events lost in a crash are not recoverable here. |
