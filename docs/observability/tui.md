...
The request-scoped `ProposeAgentConfiguration` tool exists only in that marked local turn. It records one RFC 6902 config patch and optional replacement for the existing `## Role` body; the model cannot write files. After the response settles, the host rejects stale, secret-bearing, environment-shadowed, or authority-expanding candidates, runs validation, and displays a separate approve/reject card. Approval uses canonical owner-local paths, an owner-only transaction lock, and a final config/Role comparison before atomic writes with local rollback evidence; it then replaces the responder and begins a fresh provider conversation. Secrets, runtime/model/provider posture, external MCP/plugins, channels or proactive jobs, exporters/external memory, broader tools, and weaker sandbox/network policy are handed to explicit guided flows.
