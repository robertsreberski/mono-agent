...
When no live answer stands behind the section — the event stream has dropped, a read of the listing failed, or the console is drawing a cold start from the device — it says **last known** beside the count and falls back to the last listing it heard plus the conversations this tab is holding. Only an authoritative empty listing removes the section; an unlabelled empty Running from a console that cannot see the fleet is the one claim it will not make.
