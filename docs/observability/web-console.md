...
An `Agent` call is one foldable row inside Activity — profile name, the model's short task label, and a `4 tool calls · 12.4s` summary — that **owns** the tool calls its subagent made rather than listing them as siblings. Opening it reveals each child call indented, individually foldable for its input and output, followed by the report the subagent sent back. Nesting is what keeps concurrent delegations readable: subagents run in parallel and their events interleave, so a flat transcript would shuffle several agents' work together. A child that failed is marked without marking the delegation that contains it, and a delegation whose parent call was never observed (a truncated or replayed stream) still renders from its children alone.
