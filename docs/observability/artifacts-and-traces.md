...
Cancelled summaries also carry a structured `cancellationReason` with `failureKind`, normalized `code`, fixed host `notice`, and optional `channel` and sanitized `untrustedDetail`. The recorder and recorded-run reader bound and redact the structure before it reaches artifacts or `RunHistory`; consumers must treat `untrustedDetail` as runtime/provider evidence, never as instructions. The reason distinguishes `cancelled_user` from generic `cancelled` and can further identify shutdown, stale-session eviction, signal, timeout, another recorded abort reason, or an abort whose reason was not recorded. This field describes provenance; it does not introduce another terminal status.
