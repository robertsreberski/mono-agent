...
- `run-<id>.summary.json` — a private local roll-up of the run (final `status`, aggregate usage/cost, model, and the compiled `systemPrompt` when captured). See [Run status](#run-status-and-stale-run-reconciliation) for the status values. Routed runs preserve normalized `failoverHistory` (model, failure, subkind, and request id when available). The companion events JSONL records the bounded `provider_retry_started`, `provider_failover_started`, and `provider_failover_completed` events for each attempt. Credentials and private resolver options are never copied into either artifact.
