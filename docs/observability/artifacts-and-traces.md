...
The host applies artifact retention once at startup, after stale-run reconciliation, and then on a periodic in-app sweep. Agent runs use `artifacts.retention`; memory runs use `artifacts.memoryRetention`, defaulting to a shorter 7-day / 5,000-run window. Retention deletes only terminal run summaries and their matching event files; summaries still marked `running` are never deleted by the retention pass. It does **not** delete `tool-output/<runId>` files. Tool-history retention removes only lifecycle database rows and tombstones, so no automatic cleanup owns raw saved tool output today. Operators must remove those files separately; an opaque `SessionHistory` reference then reports `available: false`. Set `dryRun: true` to log which run JSONL pairs would be pruned without removing files; memory retention inherits the agent dry-run setting when its own `dryRun` is unset.
...
the underlying file's lifetime. Automatic run-artifact retention does not remove
these raw files; if an operator or another host action removes one, later reads report the reference as unavailable. Tool-record
