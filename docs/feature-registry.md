...
| `memory.mcp` | `@mono-agent/memory-mcp` stdio MCP server exposing memory to any client: `memory_recall` (all tiers), `memory_capture` (bujo), `memory_note` (all). v2 replacement for the retired `memory.tools` mechanism | `cli` | `memory-mcp` bin; env `MONO_AGENT_MEMORY_PATH` (+ `MONO_AGENT_MEMORY_EMBEDDINGS_*`, `MONO_AGENT_MEMORY_LLM_MODEL`) — see `packages/memory-mcp/README.md` |
