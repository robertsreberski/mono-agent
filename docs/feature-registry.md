...
| `memory.bujo` | BuJo memory: SQLite-indexed daily markdown; hybrid recall (BM25+vector RRF); pipeline capture→reconcile (ADD/UPDATE/SUPERSEDE/NOOP); reflection (decay + insight synthesis); monthly migration (promote/reschedule/cluster/forget); future-log + living index | `config` | `memory.mode: "bujo"`, `memory.path` (root dir), `memory.embeddings.{provider,model,dim}`, `memory.llm.{provider,model,endpoint}` — see `docs/memory.md` |
| `memory.bujo-cli` | CLI for out-of-band BuJo maintenance: rebuild SQLite index, recall memories, reindex, reflection pass, monthly migration | `cli` | `memory-bujo rebuild|recall|index|reflect|migrate <root>` (`MONO_AGENT_MEMORY_EMBEDDINGS_*`, `MONO_AGENT_LLM_MODEL`, `MONO_AGENT_LLM_ENDPOINT` for reflect/migrate) |
