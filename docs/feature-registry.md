...
| `webhook.http-invoke` | HTTP POST invocation, sync or async with status polling; multiple named endpoints on one port, each with an optional `prompt` (pre-instructions prepended to the request text) | `config` | `webhook.enabled`, `host`, `port`, `path`, `prompt`, `defaultMode` (sync/async), `allowNonLoopback`, `retentionMs`, `maxStoredRequests`; multiple endpoints via `webhook.endpoints[]` (`name`/`path`/`mode`/`prompt`/`notify`/`enabled`) or `webhook.dir` `*.md` files (also `MONO_AGENT_WEBHOOK_ENDPOINTS_JSON`, `MONO_AGENT_WEBHOOK_DIR`) |
...
| `cron.scheduled-prompts` | Five-field cron jobs (timezone-aware, overlap-skipping) invoking the responder | `config` | `cron.jobs[]: {id, enabled, expression, timezone, prompt, conversationId, notify}` (`MONO_AGENT_CRON_JOBS_JSON` or single-job `MONO_AGENT_CRON_*`), or one `*.md` file per job in the cron folder (frontmatter metadata + prompt body) at `cron.dir` / `MONO_AGENT_CRON_DIR` (default `cron/`) — folder and config jobs merge, duplicate ids error |
| `channel.proactive-notify` | A cron job or webhook endpoint with a `notify` destination (a channel conversationId, e.g. `telegram:<chat>` or `slack:<ch>:<thread>`) is delivered as a real turn on **that channel's own harness** — not a headless run — so the message lands in the destination channel's session + history and the next live turn sees it; the channel agent composes the report and it is delivered through the channel's normal stream. The trigger is provenance-framed (`Proactive trigger from cron job "<id>". …` / `… from webhook "<name>". …`). Serializes with live traffic via the channel's per-conversation queue. **Telegram + Slack** delivery; a `whatsapp:` destination (no `notify` yet) logs a "destination does not support delivery" warning (follow-up). Slack `slack:<ch>:<thread>` targets a thread (clean continuity); bare `slack:<ch>` posts top-level (fire-and-forget, no thread continuity) | `config` | `cron.jobs[].notify` (`MONO_AGENT_CRON_NOTIFY`), `webhook.endpoints[].notify` (`MONO_AGENT_WEBHOOK_NOTIFY`); destination must be a running, allowlisted push channel |
