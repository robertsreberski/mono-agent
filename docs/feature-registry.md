...
| `runtime.context-compaction` | Context handling is delegated to the provider. The sole pi path (pi-native / pi-agent-core `AgentHarness`) has no automatic in-loop summarization, so runs report `context_compaction_applied: null` (unknown/unsupported) rather than asserting compaction ran | `provider` | Pi-native: pi-ai/AgentHarness own context handling (no auto-compaction); other bridges per their own behavior |
...
| `skills.self-authoring` | Guarded app-owned MCP tools that let the running agent persist local skill proposals under `.mono-agent/self-capabilities/proposals/` or, in apply mode with an operator approval token, create `<skillsRoot>/<name>/SKILL.md` from a saved `proposalId`, update `context.selectedSkills`, audit the write, and reload after the current turn | `config` | `selfCapabilities.{enabled,mode,skillsRoot,auditDir}` (`MONO_AGENT_SELF_CAPABILITIES_*`); default off, `mode: "propose"`; write tools require `proposalId` plus a proposal-scoped approval token derived from `MONO_AGENT_SELF_CAPABILITIES_CONFIRMATION_TOKEN` |
...
| `memory.recall-tool` | Auto-provisioned read-only `memory_recall` tool (hybrid keyword+semantic search) exposed to the agent from the single memory config; no chat LLM. `agent-app` spawns it as a bundled `mono-agent-memory` stdio child using the same memory root + embeddings as the in-app memory | `config` | `config.memory.recallTool.enabled` (`MONO_AGENT_MEMORY_RECALL_TOOL_ENABLED`, default on for journal/bujo with embeddings) |
...
| `cron.self-authoring` | Guarded app-owned MCP tools that let the running agent persist markdown cron proposals under `.mono-agent/self-capabilities/proposals/` or, in apply mode with an operator approval token, create `<cronDir>/<id>.md` from a saved `proposalId`, audit the write, and reload after the current turn | `config` | `selfCapabilities.{enabled,mode,cronDir,auditDir}` (`MONO_AGENT_SELF_CAPABILITIES_*`); cron folder still follows `cron.dir` / `MONO_AGENT_CRON_DIR` defaults; write tools require `proposalId` plus a proposal-scoped approval token derived from `MONO_AGENT_SELF_CAPABILITIES_CONFIRMATION_TOKEN` |
