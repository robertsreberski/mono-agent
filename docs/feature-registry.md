...
| `context.history` | Conversation history assembly (capped only when turns are capped) | `auto` | Sized from `runtime.maxTurns`; unlimited when `runtime.maxTurns` is omitted or `0`; custom store via `code` (`createConfiguredAgentResponder({ historyStore })`) |
| `history.persistent` | Durable per-conversation history over SQLite (`@mono-agent/history-store`), shared across channels by `conversationId` (rows isolated by channel prefix — not a cross-channel merge). Replaces the per-channel in-memory default so history survives restarts and is writable from the send-tool subprocess. On by default | `config` | `history.persist` (default `true`, `MONO_AGENT_HISTORY_PERSIST`), `history.path` (db dir; defaults to `memory.path` else `.mono-agent`, `MONO_AGENT_HISTORY_DIR`) |
| `history.proactive-send-recording` | `telegram_send_message`/`slack_send_message` record the sent message into the destination conversation's history, tagged `source: "proactive"`, so the next live turn on that channel sees what was pushed (e.g. a cron/webhook background notification). Best-effort; only actual sends are recorded. Requires persistent history + the send tool enabled | `auto` | Enabled when `history.persist` is on and the send tool is allowed (`tools.allowedTools`); daily-rollover bucketing mirrors `runtime.session.rollover`/`rolloverTimezone` |
| `history.conversations-cli` | Inspect durable history: list known conversationIds (with counts + last snippet) and show recent turns (proactive sends tagged). Useful for finding the id a cron/webhook should send to | `cli` | `mono-agent conversations list`, `mono-agent conversations show <conversationId> [--limit <n>]` |
