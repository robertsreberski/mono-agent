...
| `skills.self-authoring` | Guarded app-owned MCP tools that let the running agent propose local skills or, in apply mode with an operator token, create `<skillsRoot>/<name>/SKILL.md`, update `context.selectedSkills`, audit the write, and reload after the current turn | `config` | `selfCapabilities.{enabled,mode,skillsRoot,auditDir}` (`MONO_AGENT_SELF_CAPABILITIES_*`); default off, `mode: "propose"`; write tools require `MONO_AGENT_SELF_CAPABILITIES_CONFIRMATION_TOKEN` |
...
| `cron.self-authoring` | Guarded app-owned MCP tools that let the running agent propose markdown cron jobs or, in apply mode with an operator token, create `<cronDir>/<id>.md`, audit the write, and reload after the current turn | `config` | `selfCapabilities.{enabled,mode,cronDir,auditDir}` (`MONO_AGENT_SELF_CAPABILITIES_*`); cron folder still follows `cron.dir` / `MONO_AGENT_CRON_DIR` defaults; write tools require `MONO_AGENT_SELF_CAPABILITIES_CONFIRMATION_TOKEN` |
