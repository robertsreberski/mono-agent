...
| Recorder run-session | `@mono-agent/session-web` (its `mono-agent sessions` launcher was removed; the package and the `live` relay still ship in code) | Browser-visible recorded run with prompt, events, status, totals, and final text | Artifact retention/removal or source disappearance; the aggregator's in-memory working-set cap only evicts completed runs from memory — they stay reachable via disk paging ("Load older") — and provider rollover does not rewrite old run records |
