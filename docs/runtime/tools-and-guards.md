...
Each run collects per-turn usage, cost, and cache metrics as events for its JSONL artifact. The recorder buffers those events in memory and atomically replaces the complete events file at the terminal boundary; it does not append or checkpoint events during the run, so a crash can lose buffered data. This is automatic (coverage: `auto`) — it rides on the same `artifacts.dir` and needs no separate flag. Secrets are redacted and long strings truncated in the recorded events. See [Artifacts & traces](/observability/artifacts-and-traces/) for the complete write-boundary and stale-reconciliation contract.
