...
| `app.repo-visible-guard` | Local exact-match denylist guard for GitHub-visible titles, bodies, comments, reviews, branch names, and task metadata. The package exposes typed scanning/guard APIs for dogfood role scripts, and the CLI audits tracked local files plus optional GitHub issue/PR metadata through `gh` | `cli / code` | `@mono-agent/repo-guard`; `mono-agent repo-guard scan [--github --repo owner/name]`; `MONO_AGENT_REPO_VISIBLE_DENYLIST`, `MONO_AGENT_REPO_VISIBLE_DENYLIST_FILE` |
