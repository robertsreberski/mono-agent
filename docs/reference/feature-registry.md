...
| `app.cli-validate` | Per-section config report (core, runtime, provider credentials, context, memory, tools, sandbox, observability, every channel), optionally against a downstream consumer folder read-only | `cli` | `mono-agent validate [--consumer] [--config] [--env-file]` |
