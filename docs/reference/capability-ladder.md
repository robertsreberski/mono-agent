...
| 3 | New adapter/package in the correct package category | New package ownership, README, tests, release discovery, and catalog metadata. | Add the package to `scripts/package-catalog.mjs` with the correct `category`, `responsibility`, and `allowedDependencyCategories`; `scripts/check-package-architecture.mjs` must pass. |
| 4 | MCP server / auto-provisioned MCP tool | Runtime-visible tool lifecycle, policy/security/docs, and tool-result behavior. | Use this when the model needs an explicit callable tool boundary. Canonical app-owned examples are `memory_recall` and `notify_conversation`; arbitrary user MCP servers still belong under `tools.mcpConfigPath`. |
