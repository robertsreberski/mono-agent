...
| 4 | MCP server / auto-provisioned MCP tool | Runtime-visible tool lifecycle, policy/security/docs, and tool-result behavior. | Use this when the model needs an explicit callable tool boundary. The canonical app-owned example is `memory_recall`; arbitrary user MCP servers still belong under `tools.mcpConfigPath`. |
