...
The always-on web console deliberately remains a single-owner surface without application authentication. A fresh install binds `127.0.0.1:5050`; `--host <addr>` explicitly widens the listener, and an existing managed service keeps the bind it was published with. Anyone who can reach the listener has owner-equivalent access to retained conversations and discovered agents, so widening is a deliberate operator choice. Neither the bind nor the absence of a mono-agent-owned route proves local-only access: other proxies, tunnels, and routes are not inspected. A managed macOS `start`/`restart` re-verifies an existing owned Tailscale Serve HTTPS route and publishes a new one only with `--share-tailnet`; a failed explicit share is reported with a nonzero exit and the local console stays running (a failed route migration instead rolls the replacement worker back and says so), `web stop` removes only the exact route mono-agent owns, and no mode removes an unowned Serve handler, reverse proxy, or tunnel that already points at the port. Never publish the port directly to the internet. Host/Origin validation is a browser request-integrity guard, not an identity boundary; plain LAN HTTP is also unencrypted. Web Push adds an owner-private VAPID private key, browser endpoint/key material, and an outbox to the same resettable SQLite state. Those secrets never enter bootstrap, status, logs, or notification previews. Subscription registration is exact-origin, and outbound push endpoints are public-DNS/HTTPS validated and address-pinned to prevent the subscription API becoming an SSRF primitive. The [web console guide](/observability/web-console/#security-boundary-trusted-network-no-login) carries the operational details.
