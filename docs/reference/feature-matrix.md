...
| `memory.bujo-cli` | cli | **Removed** — the standalone `memory-bujo` bin errors and exits non-zero; use config-aware `mono-agent memory <subcommand>` from the agent folder | — | [Validation & CLI](/memory/validation-and-cli/#memory-bujo-cli--removed) | — |
...
| `live.event-relay` | config | `live.{enabled,host,port,basePath,allowNonLoopback,apiKey}` — **on by default** (read-only run-event SSE relay operator surface) | `MONO_AGENT_LIVE_*` | [Live event relay](/channels/tui/#live-event-relay-for-session-recorder) | — |
...
| `session-web.pwa` | cli | Command removed — use `mono-agent tui` (recorded-run replay) or `mono-agent web` (live console); the `@mono-agent/session-web` package still ships without a CLI launcher | `MONO_AGENT_WEB_AUTH_TOKEN` is no longer read by any code (its only reader, the removed `sessions` command, is gone); `MONO_AGENT_LIVE_*` (agent-side relay) | [CLI reference](/observability/cli-reference/#sessions) | — |
...
| `app.cli-sessions` | cli | `mono-agent sessions` command removed — use `mono-agent tui` or `mono-agent web`; the `@mono-agent/session-web` package still ships without a launcher | `MONO_AGENT_WEB_AUTH_TOKEN` is no longer read by any code (its only reader was the removed `sessions` command) | [CLI reference](/observability/cli-reference/#sessions) | — |
