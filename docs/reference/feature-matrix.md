...
| `runtime.fallback-models` | config | `runtime.fallbacks[].{model,effort?}`; legacy `runtime.fallbackModels`; CLI uses repeated `--fallback` (legacy `--fallback-models` removed in `v2.0.0`) | `MONO_AGENT_FALLBACKS_JSON`; legacy `MONO_AGENT_FALLBACK_MODELS` remains supported | [Fallback](/runtime/fallback/) | [Multi-model fallback](/playbooks/multi-model-fallback-chain/) |
...
| `runtime.per-trigger-model` | config + code | `cron.jobs[].{model,effort}`; `webhook.endpoints[].{model,effort}` + request body `{model,effort}`; Telegram built-ins `/model` and `/effort` over configured primary/fallback models | `MONO_AGENT_CRON_MODEL` / `MONO_AGENT_CRON_EFFORT`, `MONO_AGENT_WEBHOOK_MODEL` / `MONO_AGENT_WEBHOOK_EFFORT`; Telegram controls have no env key | [Cron](/channels/cron/#per-trigger-model--effort) · [Webhook](/channels/webhook/#per-trigger-model--effort) · [Telegram](/channels/telegram/#runtime-model-and-effort-controls-built-in) | — |
...
| `runtime.live-input` | code | `runtimeOptions.liveInput` (capable backends; direct OpenCode rejects) | — | [Composition](/programmatic/composition/) | — |
...
| `memory.bujo-cli` | cli | `memory-bujo rebuild\|rollback <root> --tier <lite\|journal\|bujo>`; `recall\|index\|reflect\|migrate <root>` | `MONO_AGENT_MEMORY_EMBEDDINGS_*`, `MONO_AGENT_MEMORY_LLM_MODEL`, `MONO_AGENT_MEMORY_LLM_ENDPOINT` | [Validation & CLI](/memory/validation-and-cli/) | — |
...
| `telegram.interactive` | config + code | built-in `/model` and `/effort` via adapter `runtimeControls`; `telegram.commands[]`, `telegram.reactions`, `telegram.quietHours`; `TelegramAskButtons` / `TelegramSendFile` in `tools.allowedTools` | `MONO_AGENT_TELEGRAM_REACTIONS`; runtime controls have no env key | [Telegram](/channels/telegram/) | — |
...
| `tui.stream-endpoint` | config | `tui.{enabled,host,port,basePath,allowNonLoopback,apiKey}` — **on by default** (loopback TUI/web operator surface; additive attachment capability advertised by `/v1/info`) | `MONO_AGENT_TUI_*` | [Operator stream endpoint](/channels/tui/) | — |
| `live.event-relay` | config | `live.{enabled,host,port,basePath,allowNonLoopback,apiKey}` — **on by default** (read-only SSE relay for `mono-agent sessions`) | `MONO_AGENT_LIVE_*` | [Live event relay](/channels/tui/#live-event-relay-for-session-recorder) | — |
...
| `channel.native-notify` | config | per cron job / webhook endpoint `notify`; explicit `notifyConversationId` wins, otherwise infer only with exactly one notify-capable Telegram/Slack candidate; 0 or 2+ candidates skip with a warning; cron model-exhaustion notices require explicit `notifyConversationId`, never infer, and may set `notifyFailureCooldownHours`; bounded by channel allowlists | `MONO_AGENT_CRON_NOTIFY`, `MONO_AGENT_CRON_NOTIFY_CONVERSATION_ID`, `MONO_AGENT_CRON_NOTIFY_FAILURE_COOLDOWN_HOURS` (+ webhook equivalents except cooldown) | [Delivery & send tools](/channels/delivery-and-send-tools/) | [Cron digest + native notify](/playbooks/cron-digest-proactive-notify/) |
...
| `channel.transient-tool-activity` | code | `ResilientMessageStream({ finalOnly: true, showHints: true })`; proactive delivery forces `showHints: false`; optional transport deletion supports `/cancel` cleanup | — | [Delivery & send tools](/channels/delivery-and-send-tools/) | — |
...
| `web.console` | cli | `mono-agent web [start|restart|stop|status|logs|run|reset]`; default `0.0.0.0:5050`; `--loopback`; persistent source-bound threads, streamed turns, archive/reset, browser-device attachments, conflict-safe Tailscale Serve HTTPS | — (no application auth; trusted LAN/tailnet boundary) | [Web console](/observability/web-console/) | — |
| `session-web.pwa` | cli | `mono-agent sessions [--host <addr>] [--port <n>] [--no-open] [--allow-non-loopback] [--show-auth-url] [--include-memory] [--max-runs <n>] [--config <path>] [--env-file <path>]`; unchanged read-only Session Recorder | `MONO_AGENT_WEB_AUTH_TOKEN` (recorder bearer); `MONO_AGENT_LIVE_*` (agent-side relay) | [CLI reference](/observability/cli-reference/#sessions) | — |

## Execution & composition

| Feature id | Coverage | Config key(s) | Env var(s) | Prose page | Playbook(s) |
| --- | --- | --- | --- | --- | --- |
| `app.cli-init` | cli | `--name`, exact `IDENTITY.md` → `## Role` prompt/outcome, managed project skills, `--preset` (legacy `--recipe` removed in `v2.0.0`), canonical repeated `--fallback`/`--fallback-effort` (legacy `--fallback-models` removed in `v2.0.0`), `--route-safety`, `--codex-auth`; any flag/non-TTY remains scaffold-only | — | [Quickstart](/getting-started/quickstart/) | — |
...
| `app.cli-presets` | cli | `mono-agent presets list \| show <id>` (deprecated `recipes` alias removed in `v2.0.0`) | — | [Presets & modules](/reference/recipes/) | — |
...
| `app.cli-sessions` | cli | legacy recorder command and flags moved unchanged to `mono-agent sessions` | `MONO_AGENT_WEB_AUTH_TOKEN` | [CLI reference](/observability/cli-reference/#sessions) | — |
