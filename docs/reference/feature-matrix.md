...
| `interaction.bridge` | config + auto | `interaction.bridge.{host,port}`, `interaction.askUser.timeoutMs`, `interaction.progress.enabled` | `MONO_AGENT_INTERACTION_BRIDGE_HOST`, `MONO_AGENT_INTERACTION_BRIDGE_PORT`, `MONO_AGENT_ASK_USER_TIMEOUT_MS`, `MONO_AGENT_PROGRESS_ENABLED` | [Delivery & send tools](/channels/delivery-and-send-tools/) | [Interactive transcription](/playbooks/interactive-transcription-large-media/) |
...
| `session-web.pwa` | cli | `mono-agent web [--host <addr>] [--port <n>] [--no-open] [--allow-non-loopback] [--show-auth-url] [--include-memory] [--max-runs <n>] [--config <path>]`; direct wildcard LAN/Tailscale HTTP with optional Serve for HTTPS/PWA installation; fragment bootstrap + Authorization fetch-SSE | `MONO_AGENT_WEB_AUTH_TOKEN` (web bearer, honored on any bind); `MONO_AGENT_LIVE_*` (agent-side relay) | [CLI reference](/observability/cli-reference/#web) | — |
