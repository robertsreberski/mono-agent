...
| `cron.scheduled-prompts` | config | `cron.jobs[]: {id, enabled, expression, timezone, prompt, conversationId, maxRunMs, notify, notifyConversationId}`, `cron.dir` | `MONO_AGENT_CRON_JOBS_JSON`, `MONO_AGENT_CRON_*`, `MONO_AGENT_CRON_DIR` | [Cron](/channels/cron/) | [Cron digest + native notify](/playbooks/cron-digest-proactive-notify/) |
