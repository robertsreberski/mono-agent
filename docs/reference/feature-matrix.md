...
| `tui.stream-endpoint` | config | `tui.{enabled,host,port,basePath,allowNonLoopback,apiKey}` — **on by default** (loopback TUI/web operator surface; additive attachment and verbatim-history capabilities advertised by `/v1/info`) | `MONO_AGENT_TUI_*` | [Operator stream endpoint](/channels/tui/) | — |
