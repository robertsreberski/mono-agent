...
| `agent-app.peer-agent` | config | `peers` maps names to local source IDs, and `PeerAgent` must be tool-allowed; named ACP sessions resume without prompt replay, background needs exact-origin ProcessJobs; AskUser interaction relay is unsupported | — | [ACP bridge](/programmatic/acp-bridge/) | — |
