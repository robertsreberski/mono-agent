...
| `agent-app.web-conversation-title` | auto | Request-scoped `SetConversationTitle` for writable interactive web threads; first semantic title plus material topic shifts, with permanent user-rename precedence and no trigger/archive writes. Allow-all exposes it on compatible routes; restrictive policy explicitly lists it; direct OpenCode suppresses it; no new config key | `MONO_AGENT_ALLOWED_TOOLS`, `MONO_AGENT_DISALLOWED_TOOLS` | [MCP](/tools/mcp/#setconversationtitle-web-conversation-naming) | — |
