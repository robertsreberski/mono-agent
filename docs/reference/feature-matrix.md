...
| `session-web.pwa` | cli | `mono-agent web [--host <addr>] [--port <n>] [--no-open] [--allow-non-loopback] [--show-auth-url] [--include-memory] [--max-runs <n>] [--config <path>]`; direct wildcard LAN/Tailscale HTTP with optional Serve for HTTPS/PWA installation | `MONO_AGENT_WEB_AUTH_TOKEN` (web bearer); `MONO_AGENT_LIVE_*` (agent-side relay) | [CLI reference](/observability/cli-reference/#web) | — |
