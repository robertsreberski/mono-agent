...
| `telegram.interactive` | config + code | built-in `/new`, plus `/model` and `/effort` via adapter `runtimeControls`; `telegram.commands[]`, `telegram.reactions`, `telegram.quietHours`; `TelegramAskButtons` (tap or custom text) / `TelegramSendFile` in `tools.allowedTools` | `MONO_AGENT_TELEGRAM_REACTIONS`; runtime controls have no env key | [Telegram](/channels/telegram/) | — |
...
| `channel.transient-tool-activity` | code | `ResilientMessageStream({ finalOnly: true, showHints: true })`; proactive delivery forces `showHints: false`; Telegram posts the answer separately then deletes progress; optional transport deletion also supports `/cancel` cleanup | — | [Delivery & send tools](/channels/delivery-and-send-tools/) | — |
