...
| `web.console` | cli | `mono-agent web [start|restart|stop|status|logs|run|reset]`; OS-hostname header/title/PWA identity; persisted `--theme evergreen|ocean|plum|terracotta`; default `0.0.0.0:5050`; `--loopback`; routed Chats/Board and pushed mobile thread IA; persistent source-bound threads with server-backed pins/workflow metadata/agent defaults; clustered Activity durations; device Push plus per-thread mute; exact durable structured AskUser controls; stable read-only cron channels with bounded keyset history and capability-gated confirmed actions; archive/reset; browser-device attachments; conflict-safe Tailscale Serve HTTPS; no Memory view in this release | — (no application auth; trusted LAN/tailnet boundary; cron mutations additionally require agent operator authentication and opt-in) | [Web console](/observability/web-console/) | — |
