...
| `session-web.pwa` | cli | Command removed — use `mono-agent tui` (recorded-run replay) or `mono-agent web` (live console); the `@mono-agent/session-web` package still ships without a CLI launcher | `MONO_AGENT_WEB_AUTH_TOKEN` still ships in code but is no longer reachable via any CLI command; `MONO_AGENT_LIVE_*` (agent-side relay) | [CLI reference](/observability/cli-reference/#sessions) | — |
...
| `app.cli-sessions` | cli | `mono-agent sessions` command removed — use `mono-agent tui` or `mono-agent web`; the `@mono-agent/session-web` package still ships without a launcher | `MONO_AGENT_WEB_AUTH_TOKEN` still ships in code, no longer reachable via any CLI command | [CLI reference](/observability/cli-reference/#sessions) | — |
