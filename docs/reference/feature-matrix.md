...
| `webhook.http-invoke` | config | `webhook.enabled`, `host`, `port`, `path`, `prompt`, `notify`, `notifyConversationId`, `defaultMode`, `allowNonLoopback`, `retentionMs`, `maxStoredRequests`, `webhook.endpoints[]`, `webhook.dir` | `MONO_AGENT_WEBHOOK_*`, `MONO_AGENT_WEBHOOK_ENDPOINTS_JSON`, `MONO_AGENT_WEBHOOK_DIR` | [Webhook](/channels/webhook/) | [Webhook automation (sync/async)](/playbooks/webhook-automation-sync-async/) |
...
| `cron.scheduled-prompts` | config | `cron.jobs[]: {id, enabled, expression, timezone, prompt, conversationId, maxRunMs, notify, notifyConversationId}`, `cron.dir` | `MONO_AGENT_CRON_JOBS_JSON`, `MONO_AGENT_CRON_*`, `MONO_AGENT_CRON_DIR` | [Cron](/channels/cron/) | [Cron digest + proactive notify](/playbooks/cron-digest-proactive-notify/) |
