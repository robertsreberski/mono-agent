...
`defaultChannelDrivers()` returns the core built-in channel drivers (Telegram, Slack, webhook, OpenAI API, cron, and TUI) in startup/status order. The CLI-equivalent default uses `resolveChannelDrivers(...)`, which appends external packages declared under `channels.plugins[]` such as WhatsApp, A2A, or Advisor MCP. Spread `defaultChannelDrivers()` and append your own driver for a code-only host — or expose a package-level `createChannelDriver()` and load it from config.
