...
:::caution
To withhold the on-demand memory tool from ordinary agent turns, set `config.memory.recallTool.enabled: false` — that is the switch that controls this tool, not the allowlist. Automatic score- and answer-evidence-gated context recall remains part of a configured memory backend on eligible ordinary turns. Authoritative sealed turns and host continuations are the intentional exception: they suppress automatic lookup independently of this config switch, and a seal also excludes the tool.
:::
