...
The entity graph is **bujo-only** and **auto** (coverage type: auto). It needs the chat model configured under `memory.llm` — there is no separate config switch to turn it on; it is part of the bujo capture pipeline. See [Capture and recall](/memory/capture-and-recall/) for how captures are scheduled and how recall is invoked.
...
The graph is rebuilt from the markdown files on disk by `memory-bujo rebuild`, and the entity table is rewritten by `memory-bujo index`. Recall (including one-hop expansion) can be exercised from the CLI with `memory-bujo recall`. See [Validation and CLI](/memory/validation-and-cli/) for the full subcommand reference and the `mono-agent validate` liveness checks.
