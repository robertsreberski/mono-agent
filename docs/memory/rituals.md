...
- **`index.md`** — a living table of contents: entry counts, the top/most-relevant
  ...
  referent within a bounded, deterministically ordered source window. When canonical
  rows disagree on type, the preview omits
  the type instead of choosing one. This is projection-only: canonical entity ids,
  relations, and associations are not rewritten. The consolidation result reports
  the duplicate-group count separately.
