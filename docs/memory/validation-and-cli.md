...
Lines captured before owner binding often name the owner only in their text ("The user prefers ..."), with no `person:owner` association. `curate prepare --owner-backfill` scans every live line and proposes one `person:owner` association for each line whose own source proves the owner is its subject. A line qualifies in three cases. First, it starts with `The user` or `User` followed directly by a verb, optionally after one adverb: the owner is the grammatical subject. Second, it starts with `The user's` or `User's` followed by one of the owner properties capture already accepts for owner fact labels (name, birthday, home, work and similar). Third, it already carries an owner label: a fact label about `person:owner`, or an owner-turn `agent`-scoped preference. Every other possessive (`The user's <something> ...`) names something else as the subject and is not proposed. A `User:` log envelope is not proposed either. No other id is ever proposed, and no model is called. The plan records each line's id, text hash and reason. `prepare` reports counts only: `live`, `alreadyLinked`, `notProven` (lines starting with `The user`/`User` that do not qualify), `proposed` and `bare`. Lines starting with `The user` and owner-label lines enter the plan pre-accepted as `associate:owner`. Lines starting with a bare `User` are also used for other senders, so they are listed as `associate:owner-bare` and start rejected; `review --accept associate:owner-bare` opts in (`prepare` reports their count as `bare`). `review --reject id:<memoryId>` or `--reject associate:owner` turns associations off. A line the same plan proposes to drop gets no owner association; `prepare` reports how many were left out as `supersededByDrop`. When `review` sees an accepted drop and an accepted owner association for the same line, the drop wins: the association is set to not accepted and the review summary reports the count as `ownerSupersededByDrop`. Apply refuses a line that changed, stopped qualifying, is dropped by the same plan, would no longer qualify after the same plan's rewrite or label change, or is already linked, before any backup. A canonical association stops name-derived links for its line, so apply also records that line's current name-derived associations; the line keeps every link it had. Apply creates `person:owner` when it is missing and uses the same backup, root swap, rebuild and restore as every other curation.
...
Treat every received bundle as untrusted input until `import prepare` completes successfully; do not bypass its owner, mode, identity, size, manifest, and fingerprint checks. Prepare does not mutate the memory root, but it does create the requested plan file. Apply is intentionally side-effecting: it creates a retained sibling backup, changes canonical memory and the managed index, and sends the complete merged corpus to the configured embeddings provider for re-embedding, which may involve network transfer and provider cost. `--include-extras` content is for manual operator inspection only and is never consumed by import.
...
Because import changes canonical sources, the post-import manifest advertises **no rollback generation** — exactly as after `forget apply`. The undo is `memory import restore --backup <dir>`, not `memory rollback`. Import backups are named `.<root>-import-backup-<digest>` and consume the same combined three-newest/30-day retention budget as forget backups. Copy a backup intended for longer retention outside the reserved managed names.
...
| `model` | local model string, e.g. `qwen3.6:latest` | SDK runtime ref, e.g. `openai-codex:gpt-5.6-terra` |
...
:::caution
`agent-host` memory LLMs are SDK-only for now.
:::
...
3. Check or refresh the managed memory skill. Reconcile any operator-modified
   ...
   an exact manifest-owned legacy configuration skill:
