...
The app's startup and hourly artifact-retention sweep keeps the three newest forget backups and expires snapshots after 30 days. It covers root-bound managed siblings such as `.mono-agent/.memory-forget-backup-*` and, for conventional `.mono-agent/memory`, manual `.mono-agent/operator/forget-*` directories. The sweep shares the stopped-store maintenance lease, defers while recovery is pending, never follows symlinks, and inherits `artifacts.memoryRetention.dryRun`. Copy any backup that must outlive this rollback window outside those reserved names.
