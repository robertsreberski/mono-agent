...
| `tools.web.search.backend` | `string` | `MONO_AGENT_WEB_SEARCH_BACKEND` | auto | `auto` | WebSearch backend: auto preserves configured local SearXNG, then ChatGPT-subscription Codex search, then keyless fallbacks. searxng, ollama, codex, and keyless are strict; Ollama is never silently selected by auto. |
