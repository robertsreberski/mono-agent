...
| `memory.llm.model` | `string` | `MONO_AGENT_MEMORY_LLM_MODEL` | unset | `claude:claude-sonnet-4-6` | Configures llm.model for the memory section. |
...
| `runtime.model` | `string` | `MONO_AGENT_MODEL` | required | `claude:claude-sonnet-4-6` | Configures model for the runtime section. |
...
| `slack.homeTab` | `object` | `--` | unset | `{"enabled":true}` | Slack app home tab configuration. |
...
| `slack.shortcuts` | `array` | `--` | [] | `[{"callbackId":"triage","prompt":"Triage this Slack request."}]` | Slack shortcut definitions handled by the Slack adapter. |
...
| `webhook.endpoints` | `array` | `MONO_AGENT_WEBHOOK_ENDPOINTS_JSON` | [] | `[{"id":"triage","path":"/webhook/triage","prompt":"Triage this payload."}]` | Additional named webhook endpoints beyond webhook.path. |
