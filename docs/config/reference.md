...
| `providers.piNative.cacheRetention` | `string` | `MONO_AGENT_PI_CACHE_RETENTION` | unset | `long` | Optional Anthropic Messages cache retention (short or long). Unset preserves Pi defaults/environment. Long requires model support: 1h writes cost 2× normal input, reads 0.1×; short writes cost 1.25×. No guaranteed hit. |
