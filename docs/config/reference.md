...
| `tools.web.fetch.provider` | `string | string[]` | `MONO_AGENT_WEB_FETCH_PROVIDER` | local | `local` | WebFetch provider name or ordered chain: local (default), parallel. Parallel cannot serve raw format, custom headers, or browser rendering. |
