...
| `subagents.commandTimeoutMs` | `integer` | `MONO_AGENT_SUBAGENTS_JSON` (nested field) | `1800000` | `1800000` | Positive safe integer milliseconds. Foreground Bash/Exec ceiling for detached persistent children, clamped to their own process job’s remaining runtime at child-run setup; the job deadline remains authoritative. Interactive/foreground-child caps and NodeRepl are unchanged. |
