...
| `MONO_AGENT_WEB_SEARCH_BACKEND` | `tools.web.search.backend` | One strict provider or comma-separated ordered chain. Default `parallel,ollama` (Parallel then local Ollama); names: `parallel`, `ollama`, `searxng`, `codex`, `keyless`, `duckduckgo`, `startpage`. Keyless is opt-in. `auto` is rejected with the previous explicit order. |
...
| `MONO_AGENT_WEB_FETCH_PROVIDER` | `tools.web.fetch.provider` | Strict `local` (default), `parallel`, or comma-separated ordered chain such as `local,parallel`. |
