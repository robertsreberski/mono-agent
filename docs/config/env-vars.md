...
| `MONO_AGENT_MEMORY_OPERATOR_ACTIONS_ENABLED` | `memory.operatorActions.enabled` | Owner memory edit/forget/restore through the operator endpoint; default `false`. Actions still require the actual BuJo tier and a configured `tui.apiKey`. Provider-free built-in memory reads do not require this opt-in. See [Memory operator](/memory/operator/). |
