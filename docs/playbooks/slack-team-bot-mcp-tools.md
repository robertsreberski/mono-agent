...
1. Create a Slack app with Socket Mode (app token `connections:write`), a bot token (`chat:write` plus `commands` for slash controls, and `users:read` so the agent can address teammates by name), and **Interactivity & Shortcuts** enabled so runtime selectors can deliver `block_actions` over Socket Mode. Register `/<bot-username>-model` and `/<bot-username>-effort`; Socket Mode needs no Request URL.
