...
| AUD-105 | Consecutive-crash circuit breaker for the ops fleet plists | M | P2 | #362 |
| AUD-106 | Page/notify on a tripped breaker or overdue outbox item | S | P2 | #363 |
| AUD-107 | Record a soak-end date then delete the 3 retired ops agent dirs | S | P3 | #364 |
| AUD-108 | Delete or document the empty ops `legacy/` directory | S | P3 | #365 |
| AUD-109 | Derive the expected @mono-agent version from one place in the ops fleet | S | P3 | #366 |
| AUD-110 | Surface real per-task activity in ops `./bin/agents logs` | M | P3 | #367 |
...
- **No surviving action:** #190 (pi metadata refresh), #226 (provenance portability — honestly disclosed, noted under AUD-060), #248 (repo-side shipped; open only for the external OPS fleet cutover), and epic #119 itself (closed by Bucket 0 + AUD-014's DoD text fix).
