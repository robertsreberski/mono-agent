...
- `explicit-forget.ts` (#245) — actively load-bearing, wired to the documented `mono-agent memory forget prepare|apply|restore` CLI; live-instance backup manifest (`~/example-instance/.mono-agent/.memory-forget-backup-*`) proves a real forget-and-restore cycle ran in production one day before the audit. Verifier: keep, not re-derived, no contradiction.
