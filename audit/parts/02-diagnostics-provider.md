...
**Proven dead:**
- `piLoginCommandLine` (`provider-setup.ts:251`) — `grep -rln "piLoginCommandLine"` (excl. dist/node_modules) → definition only; 0 hits in both live instances (`~/example-instance`, `~/ops-agents`). Near-dup of `piAuthRecoveryCommand`.
