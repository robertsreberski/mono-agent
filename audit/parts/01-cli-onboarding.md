...
- `wizard/run.ts:108`–`118` (`ModelRepairOutcome`) and `wizard/run.ts:336`–`388` (`runModelRepairWizard`), re-exported at `wizard/index.ts:35`–`36`. Proof: `grep -rln "runModelRepairWizard" . --include=*.ts --include=*.js --include=*.md | grep -v node_modules | grep -v /dist/` → only the definition, re-export, `wizard-run.test.ts`, `cli-first-run-state.test.ts`. Zero hits in `~/example-instance`/`~/ops-agents`. Verifier confirms externally unreachable too (exports map exposes only `.`) — the "keep as external API" hedge is moot.
