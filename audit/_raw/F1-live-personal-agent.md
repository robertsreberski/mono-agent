...
`/Users/example/.mono-agent/logs/com.mono-agent.example-instance-00000001.err.log.1` is
...
The live launchd plist (`~/Library/LaunchAgents/com.mono-agent.example-instance-00000001.plist`) runs a
...
(`100.64.0.42:4313`, `AGENTS.md:114-116`, explicitly reasoned as "Tailscale provides device auth, so
...
- **`~/example-instance/--limit/`** — a structurally valid but empty (0 rows in `messages`) wacli SQLite
...
(`agent-browser`, `agent-maintenance`, `apple-reminders`, `example-org-context`, `drafts-app`,
...
`~/Library/LaunchAgents/com.mono-agent.example-instance-00000001.plist`, `bin/agent-watchdog`,
...
`~/.mono-agent/logs/com.mono-agent.example-instance-00000001.{out,err}.log` and `.err.log.1` (grep/head/tail
...
`focus-dashboard-regen`, `focus-scan-maintenance`, `focus-scan-snapshot`, `gws-ops`, `gws-robin`,
